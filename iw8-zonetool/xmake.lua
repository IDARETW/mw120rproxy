...
target("iw8-zonetool")
    ...
    add_syslinks("bcrypt", "d3dcompiler", "d3d11", "kernel32")
...
target("css-smoke")
    ...
    add_includedirs("src")
    add_files("tests/replay_css_smoke.cpp", "src/zonetool/iw8/replay_sunshadow.cpp")

target("css-native-shaders")
    set_kind("binary")
    set_default(false)
    set_targetdir(path.join(os.projectdir(), "xmake-out", "x64", is_mode("debug") and "Debug" or "Release"))
    add_defines("WIN32", "NOMINMAX", "WIN32_LEAN_AND_MEAN")
    ...
    add_files("tests/replay_css_native_shaders.cpp")
