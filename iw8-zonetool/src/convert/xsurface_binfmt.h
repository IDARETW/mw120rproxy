// xsurface_binfmt.h — the self-describing .xsurf_bin interchange format for the xsurface family.
// =================================================================================================
// THIS IS THE A<->B CONTRACT for geometry (SPEC §3): Stage A (src/iw3/asset_xsurface.cpp) WRITES it
// from the inflated IW3 XModel.surfs[]; Stage B (src/iw8/asset_xsurface.cpp) READS it and rebuilds an
// IW8 XModelSurfs/XSurface graph. The file is wrapped by ZoneSource::wrapBlob (magic "XSURF" + u32
// version + u32 payloadLen) and lives at xsurface/<name>.xsurf_bin (ZoneSource::xsurfacePath).
//
// FORMAT IS ENGINE-NEUTRAL: it carries IW3's *unpacked* geometry (float positions/normals/uvs/tangents,
// u8 RGBA color, u16 triangle indices, rigid-vert runs, and the raw IW3 blend-weight stream) so the
// IW8 writer can repack to whatever IW8 needs without re-reading IW3 internals. All integers
// little-endian; all structs below are #pragma pack(1) so the on-disk layout is exact and host-stable.
//
// This header is owned by the xsurface family (src/convert/) and namespaced (xsurf_bin::) to avoid
// clashes with other families. It pulls in NO game headers.
// =================================================================================================
#pragma once
#include <cstdint>
...
struct FileHeader {
    uint32_t surfCount;     // number of XSurface entries (== IW3 XModel.numsurfs span we dumped)
    uint32_t nameLen;       // length of the name string INCLUDING the trailing NUL
    uint32_t partBits[8];   // DObjPartBits for the XModelSurfs (IW3 has 4 ints; we widen to 8, hi=0)
    uint32_t flags;         // reserved (0)
    uint32_t reserved[3];   // reserved (0) — keeps the header at a tidy 0x38 bytes
};

// ---- per-surface header -------------------------------------------------------------------------
// After ALL SurfHeader[surfCount] are written contiguously, each surface's payload arrays follow, in
// surface order, and within a surface in THIS field order:
//   1) GfxVertex     verts[vertCount]      (unpacked render vertex; see below)
//   2) uint16_t      indices[triCount*3]   (triangle list — 3 indices per tri)
//   3) RigidVertList rigidVertLists[rigidVertListCount]
//   4) uint16_t      blendVerts[blendVertWords]  (raw IW3 blend-weight stream; 0 when not deformed)
// (No collisionTree / lmap / tension / subdiv / blendshape data is carried — not needed for the
//  prototype's load-safe IW8 geometry. Flagged in the return notes.)
struct SurfHeader {
