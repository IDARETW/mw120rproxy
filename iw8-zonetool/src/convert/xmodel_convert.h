// xmodel_convert.h — IW3->IW8 XModel field/format conversion helpers (FOCUS family: xmodel).
// Owns the small, pure-data transforms the xmodel reader (Stage A) and writer (Stage B) both need,
// plus the canonical zone-source JSON schema (the A<->B interchange shape) so reader and writer never
// drift. Namespaced `convert::xmodel_cv` to avoid clashing with other families' helpers.
//
// SCHEMA (xmodel/<name>.json) — produced by iw3_dump_xmodel, consumed by iw8_write_xmodel:
//   {
//     "name":        string,                 // asset name
//     "numBones":    u8,  "numRootBones": u8,
//     "numsurfs":    u16,                     // surface count (== material count)
//     "numLods":     u8,  "collLod":      i8,
//     "lodRampType": u8,
//     "flags":       u32, "contents":     i32,
//     "scale":       f32, "radius":       f32,
//     "bounds":      { "midPoint":[3], "halfSize":[3] },   // IW8 Bounds (converted from IW3 mins/maxs)
//     "boneNames":   [ string, ... ],         // numBones script-string names
//     "parentList":  [ u8, ... ],             // (numBones - numRootBones) parent indices
//     "quats":       [ i16, ... ],            // 4*(numBones-numRootBones) packed tag quats
//     "trans":       [ f32, ... ],            // 3*(numBones-numRootBones) tag translations
//     "partClassification": [ u8, ... ],      // numBones (optional; empty if absent in IW3)
//     "lods": [ { "dist":f32, "numsurfs":u16, "surfIndex":u16, "partBits":[8 u32], "flags":u8 }, ... ],
//     "materials":   [ string, ... ],         // numsurfs referenced Material names (materialHandles)
//     "surfsName":   string                   // the XModelSurfs asset this model's lod0 references
//   }
// The IW3 model keeps surfaces inline; Stage A emits a sibling xmodelsurfs asset named "<name>" (or
// "<name>_lod<i>") for the geometry, and records that name here as `surfsName` so the IW8 XModel can
// cross-reference its XModelSurfs. For the prototype lod0 surfs are referenced; higher lods reuse the
// same XModelSurfs name pattern if present (xsurface family owns the actual geometry blob).
#pragma once
...
inline void boundsFromMinsMaxs(const float mins[3], const float maxs[3],
                               float midOut[3], float halfOut[3]) {
    for (int i = 0; i < 3; ++i) {
...
inline std::array<uint32_t, 8> partBitsFromIw3(const int32_t in4[4]) {
    ...
    for (int i = 0; i < 4; ++i) out[i] = static_cast<uint32_t>(in4[i]);
    return out;
}
...
inline json toJson(const Record& r) {
    ...
    for (const auto& l : r.lods) {
        lods.push_back({ {"dist", l.dist}, {"numsurfs", l.numsurfs}, {"surfIndex", l.surfIndex},
                         {"partBits", l.partBits}, {"flags", l.flags} });
    }
    j["lods"]      = lods;
...
inline bool fromJson(const json& j, Record& r) {
    try {
        ...
        if (j.contains("bounds")) {
            ...
            for (int i = 0; i < 3 && i < (int)mid.size();  ++i) r.boundsMid[i]  = mid[i];
            for (int i = 0; i < 3 && i < (int)half.size(); ++i) r.boundsHalf[i] = half[i];
        }
        ...
        if (j.contains("lods")) {
            for (const auto& lj : j["lods"]) {
                ...
                for (int i = 0; i < 8 && i < (int)pb.size(); ++i) l.partBits[i] = pb[i];
