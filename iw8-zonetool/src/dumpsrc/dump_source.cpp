// dump_source.cpp — ZoneTool (IW5) dump-folder reader. Tier-1 (map) loaders implemented; Tier-2 stubs.
// Read orders mirror zonetool-develop/src/IW5/Assets/*.cpp parse() exactly (reference/IW5_DUMP_FORMAT.md).
...
// Structs.hpp (cross-checked with IW5_DUMP_FORMAT.md). A wrong size desyncs the stream -> the loader
// catches it via BinReader::ok() and falls back to name-only + generous AABB.
namespace iw5sz {
...
DumpSource::DumpSource(const std::string& dumpDir, const std::string& mapName)
    : dir_(dumpDir), map_(mapName) {}

std::string DumpSource::sub(const std::string& rel) const { return path_join(dir_, rel); }
std::string DumpSource::mapBase() const { return sub("maps/mp/" + map_ + ".d3dbsp"); }
...
// ===================================================================================================
// TIER 1 — entityString (.ents text) + triggers (.ents.triggers bin)
// ===================================================================================================
EntsDump DumpSource::loadEnts() const {
    ...
    if (read_file(mapBase() + ".ents.triggers", tb)) {
        BinReader r(tb);
        uint32_t c;
        d.triggers.modelCount = r.read_int(); r.read_array(/*TriggerModel sizeof*/ 1, c); // size unused: count drives
        // NOTE: TriggerModel/Hull/Slab sizes are not load-bearing for mp_test (all counts 0). When a real
        // trigger set is needed, set the IW5 sizes here. We rely on the count being 0 (verified) so the
        // array tag's own count==0 yields an empty block regardless of elemSize.
        d.triggers.hullCount = r.read_int(); r.read_array(1, c);
        d.triggers.slabCount = r.read_int(); r.read_array(1, c);
...
// ===================================================================================================
// TIER 1 — ComWorld (.comworld JSON). Only name (+ lightCount) is load-bearing (com_map valid-empty).
// ===================================================================================================
ComWorldDump DumpSource::loadComWorld() const {
    ComWorldDump d;
    std::string txt;
    std::string p = mapBase() + ".comworld";
    if (!read_file_str(p, txt)) { warn("dumpsrc: no .comworld at %s", p.c_str()); return d; }
    json j;
    try { j = json::parse(txt); }
    catch (const std::exception& e) { err("dumpsrc: .comworld JSON parse: %s", e.what()); return d; }
        ...
        for (const auto& lj : j["primaryLights"]) {
            ...
            auto vec3 = [&](const char* k, float out[3]) {
                if (lj.contains(k) && lj[k].is_array() && lj[k].size() >= 3)
                    for (int i = 0; i < 3; ++i) out[i] = lj[k][i].get<float>();
            };
            vec3("color", L.color); vec3("dir", L.dir); vec3("up", L.up); vec3("origin", L.origin);
...
// ----- colmap parse_info block (mirrors ClipMap.cpp parse_info; we only need to advance the cursor) --
static void colmap_parse_info(BinReader& r) {
    ...
    for (int i = 0; i < numCBrushSides && r.ok(); ++i) {
        ...
        if (pl) r.read_array(iw5sz::cplane_s, c);
    }
    r.read_int(); r.read_array(iw5sz::cbrushedge_t, c);  // numCBrushEdges, cBrushEdges
    r.read_int(); r.read_array(iw5sz::leafBrush, c);     // numLeafBrushes, leafBrushes
    int numCLeafBrushNodes = r.read_int();
    auto nodes = r.read_array(iw5sz::cLeafBrushNode_s, c);
    for (int i = 0; i < numCLeafBrushNodes && r.ok(); ++i) {
        int16_t cnt = BinReader::geti16(nodes, (size_t)i * iw5sz::cLeafBrushNode_s + 2); // leafBrushCount@2
        if (cnt > 0) r.read_array(2, c); // brushes (u16)
    }
    ...
    for (int i = 0; i < numBrushes && r.ok(); ++i) {
        ...
        if (sidesPtr) {
            ...
            if (pl) r.read_array(iw5sz::cplane_s, c);       // sides->plane (1)
        }
        if (edgePtr) r.read_array(iw5sz::cbrushedge_t, c);  // edge (1)
    }
...
// ===================================================================================================
// TIER 1 — ClipMap (.colmap BinaryDumper). name always; full walk -> verts AABB with fallback.
// ===================================================================================================
ClipMapDump DumpSource::loadClipMap() const {
    ClipMapDump d;
    std::vector<uint8_t> buf;
    std::string p = mapBase() + ".colmap";
    if (!read_file(p, buf)) { warn("dumpsrc: no .colmap at %s", p.c_str()); return d; }

    BinReader r(buf);
    // name is the FIRST primitive — trivially correct even if the rest desyncs.
    d.name = r.read_string();
    if (!r.ok() || d.name.empty()) { err("dumpsrc: .colmap name read failed: %s", r.error().c_str()); return d; }
    ...
    r.read_int(); r.read_array(iw5sz::cLeaf_t, c);        // numCLeaf, cLeaf

    int numVerts = r.read_int();
    auto verts = r.read_array(iw5sz::VecInternal3, c);    // verts -> the AABB source
    d.numVerts = (int)c;
    debug("dumpsrc: .colmap walk: numVerts(hdr)=%d arrCount=%u cursor=%zu/%zu ok=%d",
          numVerts, c, r.pos(), buf.size(), (int)r.ok());
    ...
    if (r.ok() && c > 0 && verts.size() >= (size_t)c * iw5sz::VecInternal3) {
        ...
        if (finite) {
            ...
            for (int k = 0; k < 3; ++k) { d.boundsMin[k] = mn[k] - 64.f; d.boundsMax[k] = mx[k] + 64.f; }
    ...
    if (!r.ok()) {
        warn("dumpsrc: .colmap full walk desynced (%s); using name-only + generous AABB (Tier1-safe)",
             r.error().c_str());
...
// ===================================================================================================
// TIER 2 STUBS — clear signatures; the per-asset agents fill these using bindump.h + json.hpp.
// ===================================================================================================
MaterialDump DumpSource::loadMaterial(const std::string& nameWithPrefix) const {
    MaterialDump d; d.name = nameWithPrefix;
    // TODO(tier2-agent): parse materials/<2char>/<name> JSON per IW5_DUMP_FORMAT.md §2.
    (void)nameWithPrefix;
    return d; // loaded=false
}
XModelDump DumpSource::loadXModel(const std::string& name) const {
    XModelDump d; d.name = name;
    // TODO(tier2-agent): BinReader over XModel/<name>.xme6 per IW5_DUMP_FORMAT.md §4 (read_single<XModel>
    // header -> name -> boneNames -> ... -> per-lod read_asset<ModelSurface> -> colSurf -> phys assets).
    (void)name;
    return d; // loaded=false
}
XSurfaceDump DumpSource::loadXSurface(const std::string& surfaceName) const {
    XSurfaceDump d; d.name = surfaceName;
    // TODO(tier2-agent): BinReader over XSurface/<surfaceName>.xse per IW5_DUMP_FORMAT.md §5
    // (field-interleaved per-surface ints + verts/tris/weights arrays + collisionTree-by-stale-ptr).
    (void)surfaceName;
    return d; // loaded=false
}
GfxWorldDump DumpSource::loadGfxWorld() const {
    GfxWorldDump d;
    // DEFER: IW8 emits gfx_map valid-empty. Cheap head-only read (name + baseName) for the manifest.
    std::vector<uint8_t> buf;
    if (read_file(mapBase() + ".gfxmap", buf)) {
        BinReader r(buf);
        d.name     = r.read_string();
        d.baseName = r.read_string();
        d.loaded   = r.ok() && !d.name.empty();
    }
    return d;
}
...
std::vector<std::string> DumpSource::listMaterials() const {
    ...
    if (!list_dir(mroot, sub2)) return out;
    for (const auto& s2 : sub2) {
        std::vector<std::string> names;
        if (list_dir(path_join(mroot, s2), names))
            for (const auto& nm : names) out.push_back(s2 + "/" + nm);
    }
    return out;
}
std::vector<std::string> DumpSource::listXModels() const {
    std::vector<std::string> out, files;
    if (!list_dir(sub("XModel"), files)) return out;
    for (auto& f : files) {
        if (f.size() > 5 && f.compare(f.size() - 5, 5, ".xme6") == 0) out.push_back(f.substr(0, f.size() - 5));
    }
    return out;
}
