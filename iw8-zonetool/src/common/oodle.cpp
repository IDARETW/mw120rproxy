// oodle.cpp â€” LoadLibrary/GetProcAddress wrapper for oo2core_7_win64.dll. No link-time dependency.
#include "oodle.h"
#include "log.h"
...
typedef int64_t(__stdcall* OodleLZ_Compress_t)(int compressor,
                                               const void* rawBuf,
                                               int64_t rawLen,
                                               void* compBuf,
                                               int level,
...
typedef int64_t(__stdcall* OodleLZ_Decompress_t)(const void* compBuf,
                                                 int64_t compLen,
                                                 void* rawBuf,
                                                 int64_t rawLen,
                                                 int fuzz,
...
bool Oodle::load(const std::string& path) {
    ...
    if (!path.empty() && try_load(path)) {
        zt::info("oodle: loaded %s", path.c_str());
        return true;
    }
    ...
    if (!nextTo.empty()) {
        ...
        if (try_load(p)) {
            zt::info("oodle: loaded %s", p.c_str());
            return true;
        }
    }

    if (try_load(kDefaultPath)) {
        zt::info("oodle: loaded %s", kDefaultPath);
        return true;
    }

    g_error = "oo2core_7_win64.dll not found (tried --oodle / next-to-exe). "
              "Pass --oodle <path>.";
    return false;
}

bool Oodle::available() {
    return g_dll && g_compress && g_decompress;
}
const std::string& Oodle::last_error() {
    return g_error;
}
const char* Oodle::default_dll_path() {
    return kDefaultPath;
}

bool Oodle::compress(const std::vector<uint8_t>& in,
                     std::vector<uint8_t>& out,
                     OodleCompressor c,
                     OodleLevel level) {
    out.clear();
    if (!available()) {
        g_error = "Oodle not loaded";
        return false;
    }

    int64_t rawLen = static_cast<int64_t>(in.size());
    int64_t bound = g_bufsize
                        ? g_bufsize(static_cast<int>(c), rawLen)
                        : (rawLen + 274 * ((rawLen + 0x3FFFF) / 0x40000)); // OODLELZ_COMPRESS bound
    if (bound < rawLen + 1024)
        bound = rawLen + 1024;
    ...
    if (n <= 0) {
        g_error = "OodleLZ_Compress returned <= 0";
        out.clear();
        return false;
    }
...
bool Oodle::decompress(const std::vector<uint8_t>& in, size_t rawLen, std::vector<uint8_t>& out) {
    ...
    int64_t n = g_decompress(in.data(), static_cast<int64_t>(in.size()), out.data(),
                             static_cast<int64_t>(rawLen), 1, 0, 0, nullptr, 0, nullptr, nullptr,
                             nullptr, 0, 3);
