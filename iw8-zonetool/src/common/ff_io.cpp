...
bool iw3_parse(const std::vector<uint8_t>& file, Iw3Fastfile& out) {
    out = Iw3Fastfile{};
    if (file.size() < 12) { zt::err("iw3: file too small (%zu)", file.size()); return false; }
    ...
    out.version = ver;
    if (ver != 5) zt::warn("iw3: version %u (expected 5 for CoD4) — continuing", ver);

    // zlib stream starts at 0x0C (first byte should be 0x78).
    const uint8_t* z = file.data() + 12;
    size_t zlen = file.size() - 12;
    if (zlen == 0 || z[0] != 0x78) zt::warn("iw3: zlib stream does not start with 0x78 (0x%02X)", z[0]);
...
bool iw3_read(const std::string& ffPath, Iw3Fastfile& out) {
    ...
    if (!read_file(ffPath, file)) { zt::err("iw3: cannot read %s", ffPath.c_str()); return false; }
...
bool iw8_write(const std::string& outPath, const std::vector<uint8_t>& zoneBody,
    ...
    if (params.calcSize > params.totalDecompressed ||
        params.totalDecompressed - params.calcSize != zoneBody.size() || zoneBody.size() > UINT32_MAX - 0xA0)
    {
    ...
    if (codec == Iw8Codec::Oodle) {
        ...
        // ROUND-TRIP SELF-CHECK (workflow BLOCKER gate): the single Oodle frame MUST decompress to
        // exactly the zone body. The loader allocates XFile.size and calls OodleLZ_Decompress(rawLen=
        // XFile.size); if our align padding desyncs body.size() from XFile.size (totalDecompressed),
        // that call fails and the map won't load. Refuse to write an unloadable .ff.
        if (zoneBody.size() != bodyRawLen) {
            zt::err("iw8: %s body/XFile.size desync (%zu != %llu) — align padding not materialized "
                    ...
                    (unsigned long long)params.totalDecompressed, (unsigned long long)params.calcSize);
            return false;
        }
        std::vector<uint8_t> rtCheck;
        if (!Oodle::decompress(body, zoneBody.size(), rtCheck) || rtCheck != zoneBody) {
            zt::err("iw8: %s Oodle round-trip self-check FAILED (got %zu of %zu bytes) — not writing",
                    outPath.c_str(), rtCheck.size(), zoneBody.size());
            return false;
        }
        dashCompress = 1;
    } else if (codec == Iw8Codec::Zlib) {
        if (!deflate_all(zoneBody, body, -1)) { zt::err("iw8: zlib deflate failed"); return false; }
        dashCompress = 2; // NOTE: shipped uses 1=Oodle; 2 is our zlib path (loader leniency gamble)
    } else { // Stored
        body = zoneBody;
        dashCompress = 0;
    }
    ...
    static const unsigned char kIwcStoredFrame[4] = { 0x01, 0x49, 0x57, 0x43 }; // modeByte=1 STORED, 'I','W','C'
    ...
    if (iwc) {
        ...
        resident.insert(resident.end(), zoneBody.begin(), zoneBody.end()); // STORED: decode output == zoneBody
    } else {
        if (body.size() < 0x70) body.resize(0x70, 0); // legacy short-resident guard
    ...
    h.xfileHeader.size            = bodyRawLen;  // decode rawLen = body bytes (== zoneBody.size(); excludes calc)
    h.xfileHeader.preloadWalkSize = 0;
    for (int i = 0; i < kNumStreams; ++i) h.xfileHeader.blockSize[i] = params.blockSize[i];
    if (!iwc && params.writeSigMagic) std::memcpy(h.xfileHeader.encryption.magic, kSigMagic, 8);
    ...
    if (!f) { zt::err("iw8: cannot open %s for write", outPath.c_str()); return false; }
    ...
    const char* codecName = iwc ? "stored-IWC"
                          ...
                          : (codec == Iw8Codec::Zlib)  ? "zlib" : "stored";
    zt::info("iw8: wrote %s : %zu bytes (header 0x%zX + resident %u, codec=%s, magic=%.8s, XFile.size=0x%llX)",
...
static void inspect_iw8(const std::vector<uint8_t>& file) {
    using namespace iw8ff;
    if (file.size() < 0x8C) { zt::err("iw8: file too small for header"); return; }
    IW8_DB_FFHeader h{};
    ...
    std::printf("  dashCompressBuild  : %u (%s)\n", h.dashCompressBuild,
                h.dashCompressBuild == 0 ? "stored" : h.dashCompressBuild == 1 ? "Oodle" : "other");
...
static void inspect_iw3(const std::vector<uint8_t>& file) {
    uint32_t ver = 0;
    if (file.size() >= 12) std::memcpy(&ver, file.data() + 8, 4);
    std::printf("== IW3 fastfile (IWffu100) ==\n");
    std::printf("  magic              : %.8s\n", file.data());
    std::printf("  version            : %u %s\n", ver, ver == 5 ? "(CoD4)" : "");
    std::printf("  zlib stream @0x0C  : 0x%02X 0x%02X ...\n",
                file.size() > 12 ? file[12] : 0, file.size() > 13 ? file[13] : 0);
    std::printf("  filesize           : %zu bytes\n", file.size());
    Iw3Fastfile ff;
    if (iw3_parse(file, ff)) std::printf("  inflated zone      : %zu bytes\n", ff.zone.size());
    else std::printf("  inflated zone      : (inflate failed)\n");
}

bool inspect_ff(const std::string& ffPath) {
    std::vector<uint8_t> file;
    if (!read_file(ffPath, file)) { zt::err("inspect: cannot read %s", ffPath.c_str()); return false; }
    if (file.size() < 8) { zt::err("inspect: file too small"); return false; }
    std::printf("file: %s\n", ffPath.c_str());
    if (std::memcmp(file.data(), "IWffa100", 8) == 0 || std::memcmp(file.data(), "IWffc100", 8) == 0) { inspect_iw8(file); return true; }
    if (std::memcmp(file.data(), "IWffu100", 8) == 0) { inspect_iw3(file); return true; }
