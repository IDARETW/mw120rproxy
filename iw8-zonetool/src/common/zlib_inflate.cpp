...
bool inflate_all(const uint8_t* in, size_t inLen, std::vector<uint8_t>& out) {
    ...
    if (inflateInit(&zs) != Z_OK) { zt::err("inflateInit failed"); return false; }
    ...
    for (;;) {
        ...
        if (ret == Z_BUF_ERROR && zs.avail_in == 0) break; // truncated but no more input
    }
...
bool deflate_all(const uint8_t* in, size_t inLen, std::vector<uint8_t>& out, int level) {
    ...
    int ret = compress2(out.data(), &destLen,
                        reinterpret_cast<const Bytef*>(in), static_cast<uLong>(inLen),
                        level);
    if (ret != Z_OK) { zt::err("compress2 failed: %d", ret); out.clear(); return false; }
    out.resize(destLen);
    return true;
}
