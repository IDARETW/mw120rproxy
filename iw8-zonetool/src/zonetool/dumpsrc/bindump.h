...
class BinReader
{
  public:
    ...
    explicit BinReader(const std::vector<uint8_t> &v)
        : p_(v.data())
        , n_(v.size())
    {
    }
    ...
    int32_t read_int()
    {
        if (!expectTag(DUMP_TYPE_INT, "int"))
            return 0;
        return readScalar<int32_t>();
    }
    uint32_t read_uint()
    {
        if (!expectTag(DUMP_TYPE_INT, "uint"))
            return 0;
        return readScalar<uint32_t>();
    }
    float read_float()
    {
        if (!expectTag(DUMP_TYPE_FLOAT, "float"))
            return 0.f;
        return readScalar<float>();
    }

    // ---- string: tag 1 -> [existing byte][C-string]; tag 4 -> OFFSET back-ref
    // --------------------- Returns the NUL-terminated string (empty for NULL). `wasNull`
    // distinguishes a real empty string from a NULL marker when the caller cares.
    std::string read_string(bool *wasNull = nullptr)
    {
        ...
        {
            int32_t li, ai;
            if (!readScalarCk(li) || !readScalarCk(ai))
                return {};
    ...
    // ------------------ elemSize = sizeof(IW5 32-bit T). Returns raw bytes (count*elemSize).
    // `count` out = element count.
    std::vector<uint8_t> read_array(size_t elemSize, uint32_t &count)
    {
        ...
        if (tag == DUMP_TYPE_ARRAY)
        {
            ...
            if (cur_ + bytes > n_)
            {
                fail("read_array: %u*%zu overruns buffer", n, elemSize);
                return {};
            }
    ...
    std::vector<uint8_t> read_raw()
    {
        ...
        if (tag == DUMP_TYPE_RAW)
        {
            ...
            if (cur_ + sz > n_)
            {
                fail("read_raw: %u overruns buffer", sz);
                return {};
            }
    ...
    std::vector<uint8_t> read_single(size_t elemSize)
    {
        uint32_t c;
        return read_array(elemSize, c);
    }

    // Little-endian scalar field extractors over a raw-array element buffer (helpers for parsers).
    static int32_t geti32(const std::vector<uint8_t> &b, size_t off)
    {
        return getT<int32_t>(b, off);
    }
    static uint32_t getu32(const std::vector<uint8_t> &b, size_t off)
    {
        return getT<uint32_t>(b, off);
    }
    static uint16_t getu16(const std::vector<uint8_t> &b, size_t off)
    {
        return getT<uint16_t>(b, off);
    }
    static int16_t geti16(const std::vector<uint8_t> &b, size_t off)
    {
        return getT<int16_t>(b, off);
    }
    static uint8_t getu8(const std::vector<uint8_t> &b, size_t off)
    {
        return off < b.size() ? b[off] : 0;
    }
    static float getf(const std::vector<uint8_t> &b, size_t off)
    {
        return getT<float>(b, off);
    }
  ...
  private:
    ...
    bool readByte(uint8_t &out)
    {
        if (cur_ >= n_)
        {
            fail("eof reading byte @%zu", cur_);
            return false;
        }
        out = p_[cur_++];
        return true;
    }
    bool readTag(uint8_t &out)
    {
        return readByte(out);
    }
    bool expectTag(DumpType want, const char *what)
    {
        ...
        if (t != want)
        {
            fail("read_%s: tag %u != %u", what, t, (unsigned)want);
            return false;
        }
        return true;
    }
    template <typename T> T readScalar()
    {
        ...
        if (cur_ + sizeof(T) <= n_)
        {
            std::memcpy(&v, p_ + cur_, sizeof(T));
            cur_ += sizeof(T);
        }
        else
            fail("eof reading scalar @%zu", cur_);
        return v;
    }
    template <typename T> bool readScalarCk(T &out)
    {
        ...
        std::memcpy(&out, p_ + cur_, sizeof(T));
        cur_ += sizeof(T);
        return true;
    }
    ...
    int32_t lastString(const std::string &s)
    {
        strStore_.push_back(s);
        return (int32_t)strStore_.size() - 1;
    }
    int32_t lastArray(const std::vector<uint8_t> &a)
    {
        arrStore_.push_back(a);
        return (int32_t)arrStore_.size() - 1;
    }
    void pushList(const ListEntry &e)
    {
        list_.push_back(e);
    }
    ...
    {
        if (li < 0 || (size_t)li >= list_.size())
        {
            fail("offset string li=%d oob", li);
            return {};
        }
        const ListEntry &e = list_[li];
        if (e.kind == ENT_STRING || e.kind == ENT_ASSET)
            return strStore_[e.storeId];
    ...
    std::vector<uint8_t> offsetArray(int32_t li, int32_t ai, size_t elemSize, uint32_t &count)
    {
        ...
        if (li < 0 || (size_t)li >= list_.size())
        {
            fail("offset array li=%d oob", li);
            return {};
        }
        const ListEntry &e = list_[li];
        if (e.kind != ENT_ARRAY)
        {
            fail("offset array li=%d not an array entry", li);
            return {};
        }
        const std::vector<uint8_t> &src = arrStore_[e.storeId];
        // Re-yield from arrayIndex `ai` to the end of the source block, in elemSize units when
        // matching.
        size_t startByte = (size_t)ai * e.entrySize;
        if (startByte > src.size())
        {
            fail("offset array ai=%d oob", ai);
            return {};
        }
        std::vector<uint8_t> out(src.begin() + startByte, src.end());
        count = (elemSize && (out.size() % elemSize) == 0)
                    ? (uint32_t)(out.size() / elemSize)
