...
// ---- one rigid (single-bone) vertex run (the .xse XRigidVertList, first 8 B; collisionTree
// dropped) - IW5 XRigidVertList (Structs.hpp:771) = u16 boneOffset, vertCount, triOffset, triCount,
// then a XSurfaceCollisionTree* (4 B in the 32-bit dump). We carry only the 4 u16 run fields (IW8's
// XRigidVertList is exactly these 4 u16, 0x08). The collisionTree (render-collision accel) is NOT
// load-required for the IW8 converter and is intentionally not carried (documented in the return).
...
// Parse XSurface\<surfaceName>.xse from `xseFileBytes` (already read off disk). Returns an XseFile
// with loaded=true on a clean, fully-consumed parse; on any malformed/short stream it returns
// loaded=false with parseError set and the partial data discarded. NEVER throws.
//   - surfaceName: the asset name ("zonetool_<model>_<lod>"), used only for error messages; the
//   real
