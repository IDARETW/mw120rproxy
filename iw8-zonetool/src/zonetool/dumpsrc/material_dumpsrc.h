...
// ===================================================================================================
// READER OUTPUT — the IW5 dump material, faithfully parsed (verbatim field VALUES, no conversion
// yet).
// ===================================================================================================
struct DumpMap
{
...
// Parse a single material dump file (the absolute path to materials/<2char>/<name>) into a
// DumpMaterial. Returns d.loaded=false (and logs) on missing file / malformed JSON. `displayName`
// (optional) overrides the logged/struct name when the JSON has no "name".
DumpMaterial readMaterialJson(const std::string &filePath, const std::string &displayName = "");

// ===================================================================================================
// CONVERTER OUTPUT — an IW8-ready material: the 0x78 struct with SCALAR fields filled + the side
// tables the writer serializes (texture bindings, constants, submaterials). Pointer fields in `mat`
// are left ZEROED here; the writer stamps the tagged sentinels at emit time.
// ===================================================================================================
...
// Map the parsed IW5 dump material onto the IW8 0x78 model (scalar field conversion + slot
// assignment).
Iw8Material convert(const DumpMaterial &d);

// Serialize one converted IW8 material into the ZoneWriter as a top-level material(11) asset
// (registers via zw.add; the body runs in VIRTUAL(8) and writes the 0x78 struct + followed payloads
// in field order). Returns false (and logs) if im.ok is false.
bool writeMaterial(iw8::ZoneWriter &zw, const Iw8Material &im);

// Register the Replay HUD material used by compass_map_<map>.
...
// agree. IW5->IW8 semantic: the common low ordinals (2D/color/normal/specular) match; water(0xB)
// downgrades to color(2) (no IW8 water-material path in this converter — the bound image is treated
// as a plain image).
// ===================================================================================================
...
// True if the IW5 semantic is a water map (the bound "image" is a water_t* in-game; the dump
// already flattened it to the inner image NAME, so the reader just records the name — documented
// for the writer).
inline bool iw5_semantic_is_water(int iw5Semantic)
{
    return iw5Semantic == 0x0B;
}
// Sampler/dest slot for a texture at array position `arrayIndex` (engine default technique
// tolerates sequential binding). Kept as a function so a techset-aware pass can refine it.
