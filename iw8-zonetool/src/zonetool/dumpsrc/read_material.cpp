...
// Read a JSON value as a SIGNED int regardless of whether it was stored as int/float/null. The IW5
// dump writes sampleState as a signed char (it CAN be negative, e.g. -21 on sky_chechnya), and the
// other scalars as ints; json.hpp would throw on a get<int>() of a null, so guard nulls -> default.
...
DumpMaterial readMaterialJson(const std::string &filePath, const std::string &displayName)
{
    ...
    if (mIt != j.end() && mIt->is_array())
    {
        for (const auto &e : *mIt)
        {
            ...
            // IW5 dump uses "firstCharacter"/"lastCharacter"; tolerate the alt
            // "secondLastCharacter".
    ...
    // --- stateMap[] : [ [loadBits0, loadBits1], ... ]  (informational; IW8 owns statebits via
    // techset) ---
