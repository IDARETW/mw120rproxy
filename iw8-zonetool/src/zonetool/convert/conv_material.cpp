...
// ---- semantic
// ------------------------------------------------------------------------------------ IW5
// TextureSemantic == IW8 TextureSemantic for the common low ordinals (2D=0, color=2, normal=5,
...
{
    if (iw5Semantic == 0x0B)
        return 2; // water -> color (image treated as a plain image)
    if (iw5Semantic < 0 || iw5Semantic > 255)
        return 0;
...
// ---- sortKey
// -------------------------------------------------------------------------------------
// Render-ordering hint (NOT a load-time-walked field). Passed through unchanged, clamped to a byte.
// A future techset-aware pass can remap via a hand-built bucket table (as the IW3->IW4 path did).
...
// ---- materialType
// -------------------------------------------------------------------------------- IW8
// Material.materialType is MaterialGeometryType (u32). IW5 has no direct equivalent (it is implied
// by the techset). 0 = the generic/default geometry type, which is load-safe. (iw5GameFlags is
// unused for now; kept as the input so a future heuristic can derive a type from it.)
...
// ---- cameraRegion
// -------------------------------------------------------------------------------- IW8
// Material.cameraRegion is a GfxCameraRegionType byte. Pass the IW5 value through, clamped.
...
Iw8Material convert(const DumpMaterial &d)
{
    ...
    Material &m = out.mat;
    std::memset(&m, 0,
                sizeof(m)); // all pointer + scalar fields zeroed; writer stamps the ptr sentinels
    ...
    // _u7 (anon u16 flags @0x1A): the IW5 "stateFlags" is an 8-bit render flag set; carry it into
    // the low byte of _u7 (load-safe — not a count/pointer; purely a render hint the default
    // technique ignores).
    ...
    for (const auto &mp : d.maps)
    {
        ...
        // sampleState may be negative in the dump; reinterpret the low byte (the engine sampler
        // index is a small unsigned value; the sign is an artifact of the IW5 signed-char field).
        b.samplerState = static_cast<uint8_t>(mp.sampleState & 0xFF);
        out.textures.push_back(std::move(b));
        ++idx;
    }

    // --- constantTable: IW8 MaterialConstantDef = { index(u8), literal(vec4) } (stride 0x14). The
    // IW5
    //     dump carries {name, nameHash, literal}; IW8 has no name -> drop it, map slot to array
    //     position. ---
    for (size_t i = 0; i < d.constants.size(); ++i)
    {
        ...
        for (int k = 0; k < 4; ++k)
            c.literal.v[k] = d.constants[i].literal[k];
