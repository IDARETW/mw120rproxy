...
// every aniyah companion) the BULK content lives in STREAM 5 (region 0); small/header data in 1,2;
// large/resident-pixels in 7. This is NOT the dev/Durango 11-stream model where VIRTUAL=8 — on
// retail PC index 8 is unallocated, so a bulk write there faults the deserializer with
// Com_Error(0x1338). VIRTUAL is therefore remapped 8 -> 5. (fn-map §30a, codknowledge §20a,
// 2026-06-14.)
...
class ZoneBuffer
{
  public:
    ZoneBuffer()
        : stream_(XFILE_BLOCK_TEMP)
        , streamSize_(IW8_MAX_XFILE_COUNT, 0)
    {
    }
    ...
    // stream-load primitive sub_140001E4A610 takes the `curStream==4 -> memset(dest,0,size)` branch
    // (NOT the copy-from-body branch), so the reserved region is zero-filled at load and NOTHING is
    // copied from the Oodle body. This grows blockSize[stream_] (so DB_AllocXZoneMemory reserves a
    // real region base) but NOT buf_ — so a non-null PTR_FOLLOWS dpvs field resolves into a
