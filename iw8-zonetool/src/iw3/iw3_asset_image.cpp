...
void iw3_dump_image(iw3::LoadCtx& lc, iw3sr::ZoneSource& zs) {
    ...
    GfxImage img{};
    if (!lc.read(img)) {
        zt::err("iw3_dump_image: short read of GfxImage @%zu", structPos);
        return;
    }
...

    {
        ...
        if (texOff == LoadCtx::npos) {
            ...
            bool ok = lc.read(lcByte) && lc.read(flByte) && lc.read(&dims, sizeof(dims)) &&
                      lc.read(fmt) && lc.read(resSize);
            if (!ok) {
                zt::err("iw3_dump_image: short read of GfxImageLoadDef");
                return;
            }
            (void)ld;
            ...
            if (resSize < 0 || static_cast<size_t>(resSize) > lc.size()) {
                zt::err("iw3_dump_image: bad resourceSize %d", resSize);
                return;
            }
            pixels.resize(static_cast<size_t>(resSize));
            if (resSize > 0 && !lc.read(pixels.data(), pixels.size())) {
                zt::err("iw3_dump_image: short read of %d pixel bytes", resSize);
                return;
            }
            zt::debug("iw3_dump_image: loadDef fmt=%d %dx%dx%d L=%u res=%d", fmt, dims[0], dims[1],
                      dims[2], (unsigned)levelCount, resSize);
        } else {
    ...
    std::string name;
    {
        ...
        if (nameOff == LoadCtx::npos) {
            ...
            if (!readCStrInline(lc, name)) {
                zt::err("iw3_dump_image: short read of name");
                return;
            }
        } else {
    ...
    if (name.empty()) {
        zt::warn("iw3_dump_image: empty image name @%zu — skipping", structPos);
        return;
    }
