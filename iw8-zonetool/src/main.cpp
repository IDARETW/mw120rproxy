...
bool parse(int argc, char** argv, Args& a) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (s == "-o" && i + 1 < argc)
            a.outDir = argv[++i];
        else if (s == "--oodle" && i + 1 < argc)
            a.oodlePath = argv[++i];
        else if (s == "--stored")
            a.codec = Iw8Codec::Stored;
        else if (s == "--assets")
            a.assets = true;
        else if (s == "-v")
            zt::g_logLevel = 3;
...
bool isMapId(const std::string& map) {
    if (map.size() < 4 || map.size() > 63 || map.rfind("mp_", 0) != 0)
        return false;
...
bool validateZoneFile(const std::string& path) {
    ...
    if (header.headerVersion != iw8ff::kHeaderVersion ||
        header.xfileVersion != iw8ff::kXFileVersion) {
    ...
    if (stored ? (std::memcmp(bytes.data() + 0x88, "\x01IWC", 4) != 0 ||
                  header.xfileHeader.size != bytes.size() - 0x8C)
               : (bytes.size() < sizeof(header) ||
                  std::memcmp(header.xfileHeader.encryption.magic, iw8ff::kSigMagic, 8) != 0)) {
    ...
    for (uint64_t size : header.xfileHeader.blockSize)
        streamTotal += size;
...
bool validatePackage(const std::string& packageDir, const std::string& map) {
    ...
    if (ok)
        zt::info("validate: package '%s' passes structural checks", map.c_str());
    return ok;
}
...
struct Tier2Tally {
    int materials = 0, images = 0, xmodels = 0, xsurfs = 0;
    int matFail = 0, imgFail = 0, xmFail = 0, xsFail = 0;
};

Tier2Tally
addTier2Assets(iw8::ZoneWriter& zw, const std::string& dumpDir, const dumpsrc::DumpSource& ds) {
    Tier2Tally t;

    // --- material(11) ---
    for (const auto& rel : ds.listMaterials()) { // rel = "<2char>/<name>"
        std::string filePath = path_join(path_join(dumpDir, "materials"), rel);
        if (convdump::mtl::emitMaterialFromDump(zw, filePath, /*displayName=*/rel))
            ++t.materials;
        else
            ++t.matFail;
    }
    ...
    for (const auto& stem : dumpimg::listImageDumps(dumpDir)) {
        ...
        if (dumpimg::addImageAsset(zw, def))
            ++t.images;
        else
            ++t.imgFail;
    }
    ...
    for (const auto& name : ds.listXModels()) {
        if (iw8::addXModelFromDump(zw, dumpDir, name))
            ++t.xmodels;
        else
            ++t.xmFail;
    }
    ...
    // --- xmodelsurfs(8) : enumerate the .xse files directly (one per LOD) ---
    {
        ...
        if (list_dir(path_join(dumpDir, "XSurface"), xseFiles)) {
            for (const auto& f : xseFiles) {
                if (f.size() <= 4 || f.compare(f.size() - 4, 4, ".xse") != 0)
                    continue;
                std::string surfName = f.substr(0, f.size() - 4); // "zonetool_<model>_<lod>"
                if (iw8xs_dump::writeXModelSurfsFromDump(zw, dumpDir, surfName))
                    ++t.xsurfs;
                else
                    ++t.xsFail;
            }
...
int writeMapPackage(const Args& a,
                    const std::string& map,
                    const std::string& outDir,
                    const std::string& ents,
                    const iw8::MapBounds& bounds,
                    const std::string& dumpDir = "") {
    if (!requireMapId(map))
        return 2;
    if (a.codec != Iw8Codec::Stored) {
        zt::err(
            "build: Replay custom packages require --stored; legacy output has no valid signed auth framing");
        return 2;
    }
    mkdirs(outDir);
...

    {
        ...
        if (file_exists(lightPath)) {
            ...
            if (!std::isfinite(lighting.intensity) || lighting.intensity < 0 ||
                lighting.intensity > 4)
                throw std::runtime_error("Invalid sun intensity");
            float length = 0, upLength = 0, dot = 0;
            for (unsigned k = 0; k < 3; ++k) {
                lighting.color[k] = j.at("color").at(k).get<float>();
                lighting.direction[k] = j.at("direction").at(k).get<float>();
                lighting.up[k] = j.at("up").at(k).get<float>();
                if (!std::isfinite(lighting.color[k]) || lighting.color[k] < 0 ||
                    lighting.color[k] > 1)
                    throw std::runtime_error("Invalid sun color");
                length += lighting.direction[k] * lighting.direction[k];
                upLength += lighting.up[k] * lighting.up[k];
                dot += lighting.direction[k] * lighting.up[k];
            }
            if (!std::isfinite(length + upLength + dot) || std::abs(length - 1) > .001f ||
                std::abs(upLength - 1) > .001f || std::abs(dot) > .001f)
                throw std::runtime_error("Invalid sun basis");
        }
...

    {
        ...
        const std::string meshPath =
            !dumpDir.empty() && file_exists(meshCandidate) ? meshCandidate : "";
        ...
        if (a.assets) {
            if (dumpDir.empty()) {
                zt::warn(
                    "build: --assets set but no dump source -> main zone = gfx_map+glass_map only");
            } else {
        ...
        if (!iw8_write(out, zw.body(), p))
            rc = 1;
        zt::info("build: main zone '%s.ff' = %zu assets (gfx_map+glass_map%s)", map.c_str(),
                 zw.assetCount(), a.assets ? "+Tier2" : "+native render dependencies");
    }
    ...
    if (rc == 0 && !writeStarterManifest(outDir, map))
        rc = 1;
    if (rc == 0 && !validatePackage(outDir, map))
        rc = 1;
    if (rc == 0)
...
int cmd_build(const Args& a) {
    ...
    if (!zs.manifestRead())
        zt::warn("build: no manifest at %s (skeleton: building srv from default ents)",
                 assetDir.c_str());
...
int cmd_fromdump(const Args& a) {
    ...
    for (const auto& c : entsCandidates) {
        if (file_exists(c) && read_file_str(c, ents) && !ents.empty()) {
            zt::info("fromdump: entityString = IW8 numeric-key file %s (%zu bytes)", c.c_str(),
                     ents.size());
            haveEnts = true;
            break;
        }
    ...
    if (!haveEnts) {
        if (ed.loaded && !ed.text.empty()) {
            ...
            if (converted == 0) {
                zt::err("fromdump: could not convert a worldspawn or spawn entity from %s",
                        dumpDir.c_str());
                return 1;
            }
    ...
    if (cmd.loaded) {
        ...
        if (cmd.boundsFromVerts) {
            for (int k = 0; k < 3; ++k) {
                bounds.mn[k] = cmd.boundsMin[k];
                bounds.mx[k] = cmd.boundsMax[k];
            }
            bounds.valid = true;
            zt::info("fromdump: col_map broadphase AABB [%.1f %.1f %.1f]..[%.1f %.1f %.1f]",
                     bounds.mn[0], bounds.mn[1], bounds.mn[2], bounds.mx[0], bounds.mx[1],
                     bounds.mx[2]);
        } else {
...
int cmd_convert(const Args& a) {
    ...
    if (file_exists(dumpMarker)) {
        zt::info("convert: detected ZoneTool dump folder (%s) -> fromdump path",
                 dumpMarker.c_str());
...
int main(int argc, char** argv) try {
    ...
    if (a.cmd == "validate-package")
        return cmd_validatePackage(a);
