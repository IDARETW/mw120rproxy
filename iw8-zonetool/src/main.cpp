...
struct Args {
    ...
    std::string lightingProfile = "aniyah-incursion";
...
void usage() {
    std::printf(
        ...
        "       --lighting-profile aniyah-incursion|source (default: aniyah-incursion)\n"
        "       --sun-intensity-scale <positive multiplier> (default: 1)\n");
}
...
bool parse(int argc, char** argv, Args& a) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        else if (s == "--lighting-profile" && i + 1 < argc) a.lightingProfile = argv[++i];
        else if (s == "--sun-intensity-scale" && i + 1 < argc) {
            const std::string value = argv[++i];
            size_t consumed = 0;
            try {
                a.sunIntensityScale = std::stof(value, &consumed);
            } catch (const std::exception&) {
                return false;
            }
            if (consumed != value.size() || !std::isfinite(a.sunIntensityScale) ||
                a.sunIntensityScale <= 0)
                return false;
        }
...
int writeMapPackage(const Args& a, const std::string& map, const std::string& outDir,
    ...
    if (a.lightingProfile != "aniyah-incursion" && a.lightingProfile != "source")
        throw std::runtime_error("Unknown lighting profile: " + a.lightingProfile);
    ...
    // (Its own ZoneBuffer; never sees a Tier2 asset, so --assets cannot affect it.)
    {
        ...
        if (a.lightingProfile == "source") {
            ...
            if (!std::isfinite(length + upLength + dot) || std::abs(length - 1) > .001f ||
                (upLength != 0 && std::abs(upLength - 1) > .001f) || std::abs(dot) > .001f)
                throw std::runtime_error("Invalid sun basis");
        }
        lighting.intensity *= a.sunIntensityScale;
    ...
    if (rc == 0) {
        const auto path = path_join(outDir, "manifest.json");
...
int cmd_fromdump(const Args& a) {
    ...
    // Multi-engine adapters provide validated source bounds in a versioned file.
    ...
    if(file_exists(boundsPath)) {
        ...
        for(int k=0;k<3;++k) {
            ...
            if(!std::isfinite(bounds.mn[k]) || !std::isfinite(bounds.mx[k]) ||
               bounds.mn[k]>=bounds.mx[k] || std::abs(bounds.mn[k])>100100 || std::abs(bounds.mx[k])>100100)
