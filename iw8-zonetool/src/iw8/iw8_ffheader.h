...
struct IW8_XFile {
    ...
    uint64_t blockSize[11];    // 0x10  per-stream (DBStreamStart) decompressed sizes — IW8 has 11 streams
...
// DB_FFHeader (size 0xA0; dev struct, values confirmed against mp_shipment.ff).
struct IW8_DB_FFHeader {
    ...
    uint32_t residentHash;         // 0x18  XXH64(resident)-folded; 0 ok if db_validateloadhash off (default)
...
namespace iw8ff {
    ...
    constexpr uint32_t kResidentBase  = 0x88;     // filesize = 0x88 + residentPartSize (header proper = 0x88)
    inline constexpr char kMagic[9]    = "IWffa100";   // 'a' = SECURED (isSecured=1 -> RSA/SHA auth ring)
    inline constexpr char kMagicUnsec[9] = "IWffc100"; // 'c' = UNSECURED (isSecured=0 -> the DIRECT ?IWC decode path)
