...
void emitMapEntsBody(ZoneWriter& zw, const char* assetName, const std::string& ents) {
    ...
    zw.align(
        7); // struct -> retail STREAM 1 (live: map_ents curStream=1; map family != gfx/glass which use stream 2). [unused ZoneWriter path; kept correct]
    zw.write(me.data(), me.size());
    zw.pushStream(XFILE_BLOCK_VIRTUAL);
    zw.writeStr(assetName); // name
    zw.write(ents.data(), ents.size());
    zw.writeT<uint8_t>(0); // entityString + NUL
    zw.popStream();
    zw.popStream();
}
...
void emitColMapBody(ZoneWriter& zw, const char* assetName, const std::string& ents) {
    ...
    stamp64(cm.data(), kCM_mapEnts,
            PTR_FOLLOWS); // follows inline (duplicate MapEnts after clipMap)
    // generous world AABB (broadphase only; covers the spawns)
    stampf(cm.data(), kCM_bpMin + 0, -100000.f);
    stampf(cm.data(), kCM_bpMin + 4, -100000.f);
    stampf(cm.data(), kCM_bpMin + 8, -100000.f);
    stampf(cm.data(), kCM_bpMax + 0, 100000.f);
    stampf(cm.data(), kCM_bpMax + 4, 100000.f);
    stampf(cm.data(), kCM_bpMax + 8, 100000.f);
    stamp32(cm.data(), kCM_havokSize, 0);
    stamp64(cm.data(), kCM_havokData, PTR_NULL);
...
void emitGfxMapBody(ZoneWriter& zw, const char* assetName, const std::string& meshPath) {
    ...
    stampf(gw.data(), kGW_bounds + 0, 0.f);
    stampf(gw.data(), kGW_bounds + 4, 0.f);
    stampf(gw.data(), kGW_bounds + 8, 0.f);
    stampf(gw.data(), kGW_bounds + 12, 100000.f);
    stampf(gw.data(), kGW_bounds + 16, 100000.f);
    stampf(gw.data(), kGW_bounds + 20, 100000.f);
    ...
    zw.writeStr(
        assetName); // baseName = raw "maps/mp/mp_test.d3dbsp\0" (own copy; struct stamp = PTR_FOLLOWS)
    ...
    zw.align(7);
    {
        ...
        stampf(cell.data(), 0, 0.f);
        stampf(cell.data(), 4, 0.f);
        stampf(cell.data(), 8, 0.f);
        stampf(cell.data(), 12, 100000.f);
        stampf(cell.data(), 16, 100000.f);
        stampf(cell.data(), 20, 100000.f);
    ...
    zw.popStream();
    uint8_t tree[48]{};
    stampf(tree, 12, 100000.f);
    stampf(tree, 16, 100000.f);
    stampf(tree, 20, 100000.f);
...
std::string guardKey(const iw8::ZoneWriter& zw, const std::string& name) {
    char buf[32];
    std::snprintf(buf, sizeof(buf), "%p:", (const void*)&zw);
...
void iw8_write_maps(iw8::ZoneWriter& zw, iw3sr::ZoneSource& zs, const char* name) {
    ...
    zt::info(
        "iw8 maps: registered map_ents+col_map+com_map+glass_map+gfx_map for '%s' (ents=%zu bytes)",
