// iw8_structs.h — IW8 (MW2019 1.24) zone-asset structs + the authoritative asset-type/size tables.
// Layouts dev.i64-confirmed; ordinals from retail g_assetNames @0x4A9D090; sizes from retail
// g_assetSizes @0x4A9D7F0 (fastfile_research/14b §5). Every asset struct the writer emits MUST be
// exactly g_assetSizes[type] bytes — static_assert it (SPEC §6.2 / §8.2).
#pragma once
#include <cstdint>
#include <cstddef>
...
// ---- XAssetType ordinals (retail-confirmed, fastfile_research/14b §5a) --------------------------
...
namespace iw8sz {
    ...
    // String name for the CSV manifest (SPEC §3). Keep in sync with IW8_XAssetType.
    inline const char* type_name(int t) {
        switch (t) {
            case ASSET_TYPE_XMODELSURFS: return "xmodelsurfs";
            case ASSET_TYPE_XMODEL:      return "xmodel";
            case ASSET_TYPE_MATERIAL:    return "material";
            case ASSET_TYPE_IMAGE:       return "image";
            case ASSET_TYPE_TECHSET:     return "techset";
            case ASSET_TYPE_COL_MAP:     return "col_map";
            case ASSET_TYPE_COM_MAP:     return "com_map";
            case ASSET_TYPE_GLASS_MAP:   return "glass_map";
            case ASSET_TYPE_MAP_ENTS:    return "map_ents";
            case ASSET_TYPE_FX_MAP:      return "fx_map";
            case ASSET_TYPE_GFX_MAP:     return "gfx_map";
            case ASSET_TYPE_LIGHTDEF:    return "lightdef";
            case ASSET_TYPE_RAWFILE:     return "rawfile";
...
// ===================================================================================================
// FOCUS ASSET STRUCTS — field maps to be recovered from dev.i64 (full PDB) + validated vs
// mp_shipment.ff real instances (SPEC §4c). Until pinned, writers may emit fixed-size zeroed buffers
// with named-offset stamps (the map_zone.h pattern). When a full struct is added here it MUST
// static_assert(sizeof == iw8sz::<TYPE>).
//   TODO(idat-agent): IW8_GfxImage  (0xE8)  + GfxImageLoadDef
//   TODO(idat-agent): IW8_Material  (0x78)  + MaterialTextureDef
//   TODO(idat-agent): IW8_XModel    (0x2B0)
//   TODO(idat-agent): IW8_XModelSurfs (0x60) + XSurface
//   (clipMap_t / MapEnts / ComWorld / GfxWorld head offsets: see map_zone.h + SPEC §4c)
// ===================================================================================================
