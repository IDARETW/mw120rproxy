// iw8_map_structs_pinned.h
// IW8 (MW2019) 1.24 LOAD-CRITICAL map asset structs — pinned field maps with sizeof == g_assetSizes.
// Companion to iw8_map_structs_pinned.md. Recovered 2026-06-14 from _ida_tmp/dev.i64 (IW8_DEV 8.24
// full PDB) + validated vs the real retail GfxWorld instance in
// fastfile_research/dumps/maps_mp_mp_m_overunder.d3dbsp.gfx_map.txt and the retail g_assetSizes table.
//
// Layout rule: pad-to-offset members keep every field at its EXACT struct offset; the trailing
// static_assert pins total sizeof to the retail g_assetSizes value (the hard load invariant). For the
// big embedded sub-structs we don't field-map (MapTriggers inner ptrs, GfxWorldDraw, etc.) we reserve
// a raw byte span so offsets after them stay correct.
//
// HARD INVARIANT: every struct below sizeof() == g_assetSizes[type]. If a static_assert fires, the
// field map is wrong (a load over/under-copy crash).

#pragma once
#include <cstdint>
#include <cstddef>

// Integrated into the build (was reference/iw8_map_structs_pinned.h). Namespaced under iw8::maps so the
// pinned field maps + static_asserts compile-validate the load-critical sizes/offsets WITHOUT colliding
// with the offset-stamp constants in map_zone.h. The map zone emitter cross-references these offsets.
namespace iw8 { namespace maps {
...
struct Bounds { vec3_t midPoint; vec3_t halfSize; }; // 24

// ===========================================================================================
// clipMap_t  — col_map (23) — g_assetSizes 0xF8 (248). CONFIRMED(PDB), dev==retail.
// ===========================================================================================
struct MapTriggers {                 // 0x50 (reused in clipMap_t.stageTrigger and MapEnts.trigger)
    uint32_t count;          // 0x00
    uint32_t _pad04;
...
// ===========================================================================================
struct SpawnPointRecordList { uint32_t count; uint32_t _pad; void* records; }; // 0x10

struct MapEnts {                     // 0x428
    const char* name;                // 0x00  PTR
...
// dev PDB sizeof = 0x41E0 → retail is +0x3F0 LARGER (inserted in the post-`draw` light/dpvs region).
// HEAD (name..draw) is byte-identical dev<->retail (CONFIRMED against the real mp_m_overunder
// instance). The tail is opaque-padded so sizeof == 0x45D0 and checksum lands at the proven retail
// offset 0x3E60 (= dev 0x3A70 + 0x3F0; gfx[0x3E60]==col_map[0xF0] over 3 real samples).
//
// ⚠ RENDER-SAFETY: an ALL-ZEROS GfxWorld is NOT render-safe — R_SetupDpvsForPoint does
// `for(i=*dpvsPlanes.nodes; ...)` with NO null/count guard, so nodes==NULL => crash. Tier1: either
...
// ===========================================================================================
struct GfxWorldDpvsPlanes {          // 0x28  (render-critical) — CONFIRMED(instance)
...
//   GFXWORLD_RETAIL_CHECKSUM_OFFSET = 0x3E60  (proven: gfx[0x3E60] == col_map[0xF0], 3 real samples)
