// write_image.cpp — Stage-B: serialize an Iw8ImageDef into the IW8 ZoneWriter as a GfxImage(19)=0xE8.
// =================================================================================================
// The DUMP-PATH image writer (distinct from the legacy IW3-.ff convert::iw8_write_image in
// src/iw8/asset_image.cpp). Stamps the fixed 0xE8 GfxImage struct via the PINNED field offsets
// (iw8_focus::GfxImage / reference/iw8_asset_structs_pinned.h) and emits it in LOAD-READ-ORDER with the
// raw IW8 tagged-pointer sentinels, exactly like the proven map_zone.h pattern.
//
// PROTOTYPE IMAGE MODEL (SPEC §3a/§4 "inline pixel buffer, no .xpak"):
//   - name @0x00          -> -2 (follows): the true asset name is written inline right after the struct.
//   - packedAtlasData @08 -> null.
//   - streams[4] @0x38    -> zeroed; streamedPartCount=0 => the loader treats the image as fully
//                            RESIDENT (no imagefile*.pak we cannot supply offline).
//   - fallback @0xD8      -> null.
//   - pixels @0xE0        -> -2 (follows) when an inline payload exists (DXT/RGBA bytes written right
//                            after the name), else null. (GfxImagePixels is an 8B pointer union; -2
//                            means "the resident pixel bytes follow inline".)
//
// STREAM PLACEMENT (dev.i64 Load_GfxImagePtr/Load_GfxImagePixels): struct -> TEMP_PRELOAD(1) 16-align;
// name -> VIRTUAL(8); resident pixels -> UNK7(7) 256-align (non-streamed branch, flags&0x40==0).
//
// SIZE INVARIANT: we stamp into a fixed kImageSize(0xE8) buffer via literal offsets that are
// static_assert-bound to iw8_focus::GfxImage, so the emitted struct is exactly g_assetSizes[19]=0xE8
// bytes regardless of host padding.
//
...
static_assert(offsetof(iw8_focus::GfxImage, streamedPartCount) == kGI_streamedPart, "streamedPart@0x31");
...
bool emitImageBody(iw8::ZoneWriter& zw, const Iw8ImageDef& def) {
    if (def.name.empty()) { err("dumpimg: emitImageBody: empty image name"); return false; }
    ...
    s32(gi, kGI_format,       def.format);                         // GfxPixelFormat (provisional map)
    ...
    // 2) emit in load-read order (dev.i64 Load_GfxImagePtr/Load_GfxImagePixels — fixed per workflow
    //    BLOCKER/HIGH): struct -> TEMP_PRELOAD(1) 16-align; name -> VIRTUAL(8); pixels -> UNK7(7) 256-align.
...
bool addImageAsset(iw8::ZoneWriter& zw, const Iw8ImageDef& def) {
    if (def.name.empty()) { err("dumpimg: addImageAsset: empty image name"); return false; }
...
size_t buildImageZone(iw8::ZoneBuffer& zb, const std::vector<Iw8ImageDef>& defs) {
    ...
    al.assetCount = (uint32_t)defs.size(); al.assetReadPos = 0; al.assets = PTR_FOLLOWS;
    zb.writeT(al);
    zb.popStream();
    ...
    for (size_t i = 0; i < defs.size(); ++i) {
        IW8_XAsset a{}; a.type = ASSET_TYPE_IMAGE; a.header = PTR_INSERT; zb.writeT(a);
    }
    ...
    for (const auto& def : defs) {
        ...
        zb.pushStream(XFILE_BLOCK_TEMP_PRELOAD); zb.align(15);   // struct 16-align
        zb.write(gi, sizeof(gi));
            zb.pushStream(XFILE_BLOCK_VIRTUAL);
            zb.writeStr(def.name.c_str());                       // name -> VIRTUAL(8)
            zb.popStream();
            if (pixSize) { zb.pushStream(XFILE_BLOCK_UNK7); zb.align(0xFF); zb.write(def.pixels.data(), pixSize); zb.popStream(); }
        zb.popStream();
    }
