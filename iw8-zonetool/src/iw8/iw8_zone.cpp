// iw8_zone.cpp — ZoneWriter::build() framing + buildEmptyZone().
// Mirrors ZONE_LOAD_MODEL.md §2/§3 and the proven iw8zonewriter main.cpp ordering:
//   1) XAssetList -> TEMP(0): {stringList{0,0,0}, assetCount, assetReadPos=0, assets=-2|0}
//   2) XAsset[N]  -> VIRTUAL(8) (align 7): each {type, _pad, header=-3 insert}
//   3) per asset: invoke the registered body callback (writes struct + trailing data)
#include "iw8_zone.h"
...
void ZoneWriter::build() {
    ...
    al.assetCount = n; al.assetReadPos = 0;
    al.assets = (n > 0) ? PTR_FOLLOWS : PTR_NULL;
    zb_.writeT(al);
    zb_.popStream();

    if (n == 0) { zt::info("iw8 zone: 0 assets (valid-empty)"); return; }
    ...
    for (const auto& e : entries_) {
        IW8_XAsset xa{};
        xa.type = e.type; xa._pad04 = 0; xa.header = PTR_INSERT;
        zb_.writeT(xa);
    }
    ...
    for (const auto& e : entries_) {
        ...
        zb_.align(7); zb_.reserveCalc(8);
        if (e.body) e.body(*this);
    }
...
void buildEmptyZone(ZoneBuffer& zb) {
    ...
    IW8_XAssetList al{};
    al.stringList.count = 0; al.stringList.loaded = 0; al.stringList.strings = PTR_NULL;
    al.assetCount = 0; al.assetReadPos = 0; al.assets = PTR_NULL;
    zb.writeT(al);
    zb.popStream();
}
