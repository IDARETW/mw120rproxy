...
static bool parseBin(const std::vector<uint8_t>& payload, ParsedFile& out) {
    ...
    auto need = [&](size_t n) -> bool {
        return p + n <= payload.size();
    };
    auto take = [&](void* dst, size_t n) -> bool {
        if (!need(n))
            return false;
        std::memcpy(dst, payload.data() + p, n);
        p += n;
        return true;
    };

    xsurf_bin::FileHeader fh{};
    if (!take(&fh, sizeof(fh)))
        return false;
    ...
    for (uint32_t i = 0; i < fh.surfCount; ++i) {
        if (!take(&out.surfs[i].hdr, sizeof(xsurf_bin::SurfHeader)))
            return false;
    }
    for (uint32_t i = 0; i < fh.surfCount; ++i) {
        ...
        if (h.vertCount) {
            ...
            if (!take(s.verts.data(), (size_t)h.vertCount * sizeof(xsurf_bin::GfxVertex)))
                return false;
        }
        if (h.triCount) {
            s.indices.resize((size_t)h.triCount * 3);
            if (!take(s.indices.data(), s.indices.size() * sizeof(uint16_t)))
                return false;
        }
        if (h.rigidVertListCount) {
            s.rigid.resize(h.rigidVertListCount);
            if (!take(s.rigid.data(),
                      (size_t)h.rigidVertListCount * sizeof(xsurf_bin::RigidVertList)))
                return false;
        }
        if (h.blendVertWords) {
            s.blendVerts.resize(h.blendVertWords);
            if (!take(s.blendVerts.data(), (size_t)h.blendVertWords * sizeof(uint16_t)))
                return false;
        }
    }
    return true;
...
static void writeMinimal(ZoneWriter& zw, const std::string& name) {
    ...
    zw.popStream();
    zw.popStream();
    zt::warn("iw8 xsurface '%s': emitted minimal name-only XModelSurfs (no geometry)",
             name.c_str());
}
...
bool writeXModelSurfs(ZoneWriter& zw, iw3sr::ZoneSource& zs, const std::string& name) {
    ...
    if (!iw3sr::ZoneSource::unwrapBlob(blob, magic, ver, payload) || magic != xsurf_bin::MAGIC) {
    ...
    for (uint16_t i = 0; i < numsurfs; ++i) {
        ...
        float mid[3], half[3];
        computeBounds(s.verts, mid, half);
        ...
        if (!s.indices.empty())
            appendBlob(s.indices.data(), s.indices.size() * sizeof(uint16_t));
    }
    align16();
    ...
    std::memset(&xs, 0, sizeof(xs));
    {
        ...
        std::memcpy(reinterpret_cast<uint8_t*>(&xs) + 0x30, (sharedDataSize ? &followsN : &nullN),
                    8); // shared (follows|null)
    }
