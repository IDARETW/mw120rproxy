...
bool writeXModelSurfsFromDump(ZoneWriter& zw, const std::string& dumpDir, const std::string& name) {
    ...
    if (!xse.loaded) {
        zw.add(ASSET_TYPE_XMODELSURFS, name, [name](ZoneWriter& w) {
            emitMinimalBody(w, name);
        });
        return false;
    }
    ...
    zw.add(ASSET_TYPE_XMODELSURFS, name, [name, cv](ZoneWriter& w) {
        emitFullBody(w, name, cv);
    });
    return true;
}
