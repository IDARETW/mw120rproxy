// iw8_focus_structs.h — EXACT IW8 (MW2019 1.24) FOCUS asset field maps for the per-asset writers.
// =================================================================================================
// Merged from reference/iw8_focus_structs_pinned.h (dev.i64 full-PDB pin, sizes cross-checked vs
// retail g_assetSizes). See reference/IW8_FOCUS_STRUCTS.md for provenance/confidence per struct.
//
// PROVENANCE TAGS (carried from the pin):
//   [PDB]  full-PDB dev.i64 struct dump (name+offset+type from the compiler's own type info).
//   [TBL]  retail g_assetSizes table @ RVA 0x4A9D7F0 (the asset sizeof invariant).
//   [LOAD] confirmed by a dev Load_* decompile (e.g. array stride from Load_Stream(..,N*count)).
//   [INFER] inferred / not directly pinned — flagged inline.
//
// ALIGNMENT NOTE — these structs use **natural alignment** (the dev field offsets reproduce under the
// compiler's default packing). They are therefore deliberately defined OUTSIDE of iw8_structs.h's
// `#pragma pack(push,1)` region. iw8_structs.h pops its pack before this header's body runs (we only
// include it for the iw8sz:: size constants), so default 8-byte alignment is in effect here.
//
// SIZE INVARIANT — each FOCUS asset struct static_asserts BOTH its pinned literal size AND the matching
// iw8sz::<TYPE> g_assetSizes constant (they are the same number; the dual assert keeps the two sources
// wired together). Sub-structs that have no g_assetSizes entry assert only their pinned literal size.
//
// GfxImageLoadDef: DOES NOT EXIST in IW8 (PDB search returned zero names). IW8 replaced the IW3
// inline-pixels GfxImageLoadDef with GfxImage.streams[4] (GfxImageStreamData) + GfxImage.pixels.
// Documented here, intentionally not emitted as a type.
//
// POINTER CONVENTION (ZONE_LOAD_MODEL.md): every //PTR field is a zone pointer — at write time stamp
// the tagged sentinel (-2 follows / -3 insert / -1 shared / 0 null), NOT a real address. Every //CNT
// field is the array-count governing how many elements the matching //PTR points to.
// =================================================================================================
#pragma once
#include <cstdint>
...
namespace iw8_focus
{
    ...
    struct vec4_t   { float v[4]; };         // [PDB] size 0x10
    struct Bounds   { vec3_t midPoint; vec3_t halfSize; }; // [PDB] 0x18

    // forward decls
    struct GfxImage; struct Material; struct MaterialTechniqueSet; struct GfxImageFallback;
    struct XModelSurfs; struct XSurface; struct XSurfaceShared; struct XModelLodInfo;
    ...
    struct GfxImage
    {
        ...
        // pointer). //PTR-ish (runtime/streamed). For an inline-pixels prototype, the actual DXT
    ...
    static_assert(sizeof(XModel) == iw8sz::XMODEL, "XModel size must equal retail g_assetSizes[9]=0x2B0");
    ...
    static_assert(sizeof(XModelSurfs) == iw8sz::XMODELSURFS, "XModelSurfs size must equal g_assetSizes[8]");
