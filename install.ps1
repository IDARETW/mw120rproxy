...
$configName = if ($Mode -eq 'debug') { 'Debug' } else { 'Release' }
...
if (-not (Test-Path -LiteralPath $sourceRoot)) { throw "Source root is missing: $sourceRoot" }
if (-not (Test-Path -LiteralPath $gameExe)) { throw "Game executable is missing: $gameExe" }

if (!$SkipBuild) {
Push-Location $sourceRoot
try {
    & xmake f -m $Mode -a x64 -y
    if ($LASTEXITCODE -ne 0) { throw "XMake configuration failed with exit code $LASTEXITCODE." }
    & xmake
...
if ((Get-Item -LiteralPath $artifact).Length -lt 64KB) { throw "Build artifact is too small: $artifact" }
...
if ($LASTEXITCODE -ne 0) { throw 'Target/export verification failed; no deployment.' }
if (Get-TargetGameProcess) { throw 'Close Replay before deploying. The script never starts or stops the game.' }
...
if ((Get-FileHash -LiteralPath $staged).Hash -ne (Get-FileHash -LiteralPath $artifact).Hash) { throw 'Staging hash mismatch.' }
if (Get-TargetGameProcess) { throw 'Replay started during staging; deployment deferred.' }
try {
    ...
    if ((Get-FileHash -LiteralPath $destination).Hash -ne (Get-FileHash -LiteralPath $artifact).Hash) { throw 'Deployed hash mismatch.' }
} catch {
    ...
    if (Test-Path -LiteralPath $previousIni) { Copy-Item -LiteralPath $previousIni -Destination (Join-Path $gameRoot 'mw120rproxy.ini') -Force }
    throw
}
[pscustomobject]@{Deployed=$destination;SHA256=(Get-FileHash -LiteralPath $destination).Hash;Backup=$backup;GameStarted=$false} |
