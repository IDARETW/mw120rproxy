...
bool CompleteLocalAuth(uintptr_t object) {
    ...
    if (g_repairs.fetch_add(1) < 8)
        LOG_INFO(
            "Auth",
            "%s local controller 0: bnet=%d->2 prior_error=%08X xuid=%llu name='%s' ownership=1",
...
bool ConnectPlatformClient(uintptr_t object) {
    ...
    if (g_enabled.load(std::memory_order_acquire) && object == g_base + kPlatformClient) {
        if (g_connectSkipped.fetch_add(1) == 0)
            LOG_INFO(
                "Auth",
                "offline startup: skipped external Battle.net client connection before BeginAuth");
        return false;
    }
...
void Begin(uintptr_t object, uintptr_t a2, uintptr_t a3, uintptr_t a4) {
    ...
    if (g_enabled.load(std::memory_order_acquire)) {
        if (!CompleteLocalAuth(object)) {
            ...
            if (g_repairs.fetch_add(1) < 8)
                LOG_ERR("Auth",
                        "BeginAuth object/ranges do not match Replay; forwarding stock call");
        }
    }
...
bool PlatformId(int controller, uint64_t* out) {
    if (!Ready(controller))
        return g_platformId.load(std::memory_order_acquire)(controller, out);
...
bool PlatformName(int controller, char* out, size_t size) {
    if (!Ready(controller))
        return g_platformName.load(std::memory_order_acquire)(controller, out, size);
    if (!size || size > 4096 || !Writable(reinterpret_cast<uintptr_t>(out), size))
        return false;
...
const char* Error(const uint32_t* code) {
    if (g_errorCalls.fetch_add(1) < 8) {
        ...
        LOG_ERR("Auth", "native Blizzard error BLZBNTBGS%08X; local_ready=%d", value,
                int(g_ready.load()));
        void* frames[12]{};
        ...
        for (USHORT i = 0; i < count; ++i) {
            ...
            if (pc >= g_base && pc < g_base + 0x1324B000)
                LOG_ERR("Auth", "  frame %u game RVA 0x%llX", i,
                        static_cast<unsigned long long>(pc - g_base));
        }
        LogSnapshot();
    }
...
hook::Status UseStockOfflineBootCallback() {
    const uint8_t trueCode[] = {0x40, 0x53, 0x48, 0x83, 0xEC, 0x20, 0xBA,
                                1,    0,    0,    0,    0x48, 0x8B, 0xD9};
    if (!Matches(kStockTrue, trueCode))
    ...
    if (current != reinterpret_cast<uintptr_t>(expected))
        return hook::Status::NotReady;
    DWORD old = 0;
    ...
    if (prior != expected || !restored)
        return hook::Status::Failed;
    LOG_INFO(
        "Auth",
        "offline boot: Engine.IsDemoBuild uses stock true callback; Local Play already allowed in Replay");
    return hook::Status::Installed;
}
}
...
bool Prepare(HMODULE self) {
    ...
    if (!slash)
        return false;
    if (wcscpy_s(slash + 1, MAX_PATH - (slash + 1 - path), L"mw120rproxy.identity.ini"))
        return false;
    ...
    if (attributes == INVALID_FILE_ATTRIBUTES) {
        ...
        if (file == INVALID_HANDLE_VALUE)
            return false;
        DWORD written = 0;
        const bool saved = WriteFile(file, text, count, &written, nullptr) &&
                           written == static_cast<DWORD>(count) && FlushFileBuffers(file);
        CloseHandle(file);
        if (!saved)
            return false;
    }
    ...
    if (!g_identity.name[0])
        strcpy_s(g_identity.name, "LocalPlayer");
    g_prepared = true;
    LOG_INFO("Auth",
             "persistent local identity loaded; xuid=%llu name='%s' (local MP compatibility)",
             static_cast<unsigned long long>(g_identity.xuid), g_identity.name);
    return true;
}
...
hook::Status Install(uintptr_t base) {
    ...
    const uint8_t userGetter[] = {0x48, 0x63, 0xC1, 0x48, 0x69, 0xC8, 0xE8, 0,   0,
                                  0,    0x48, 0x8D, 0x05, 0x8F, 0xAD, 0xF5, 0x0C};
    const uint8_t xuidGetter[] = {0x48, 0x8B, 0x05, 0xD1, 0x50, 0x9F, 0x0D,
                                  0x48, 0x89, 0x01, 0x48, 0x8B, 0xC1, 0xC3};
    const uint8_t clientGetter[] = {0x48, 0x8D, 0x05, 0x49, 0x7C, 0x9F, 0x0D, 0xC3};
    if (!Matches(0x1660280, getter) || !Matches(0x1665990, userGetter) ||
        !Matches(0x1665C10, xuidGetter) || !Matches(0x1662FC0, clientGetter))
        return hook::Status::NotReady;

    // Complete instruction boundaries verified in the MD5-matching Replay image.
    const uint8_t begin[] = {0x40, 0x53, 0x48, 0x83, 0xEC, 0x50, 0x48,
                             0x8B, 0x05, 0x4B, 0x44, 0x3F, 0x04};
    const uint8_t signedIn[] = {0x40, 0x53, 0x48, 0x83, 0xEC, 0x20, 0x8B, 0xD9};
    const uint8_t status[] = {0x40, 0x53, 0x48, 0x83, 0xEC, 0x20, 0x48,
                              0x63, 0xC1, 0xBA, 0x21, 0,    0,    0};
    ...
    const uint8_t error[] = {0x40, 0x53, 0x48, 0x81, 0xEC, 0xB0, 0,    0,
                             0,    0x48, 0x8B, 0x05, 0xF8, 0xF9, 0x3E, 0x04};
    const uint8_t connect[] = {0x48, 0x89, 0x5C, 0x24, 8,    0x48, 0x89,
                               0x7C, 0x24, 0x10, 0x55, 0x48, 0x8B, 0xEC};
#define AUTH_HOOK(rva, fn, bytes, original)                                                        \
    {                                                                                              \
        const auto s = hook::Install(reinterpret_cast<void*>(base + rva), fn, bytes,               \
                                     sizeof(bytes), original);                                     \
...
#undef AUTH_HOOK
    ...
    if (boot != hook::Status::Installed)
        return boot;
    g_enabled.store(true, std::memory_order_release);
    LOG_INFO(
        "Auth",
        "8 verified Replay hooks installed; external client wait disabled for local startup; waiting for BeginAuth RVA 0x165FFA0");
    return hook::Status::Installed;
}
...
void LogSnapshot() {
    ...
    LOG_INFO(
        "Auth",
        "snapshot: ready=%d begin=%llu dw_queries=%llu stats_queries=%llu bnet=%d user=%d identity_match=%d finished=%u owns=%d content=%u offline_stats_fetched=%u error=%08X",
        int(g_ready.load()), g_beginCalls.load(), g_dwCalls.load(), g_statsCalls.load(),
        Read<int>(kAuth), Read<int>(kUser),
        int(Read<uint64_t>(kUser + 0x90) == g_identity.xuid &&
            Read<uint64_t>(kUser + 0xB8) == g_identity.xuid &&
            Read<uint64_t>(kPlatformXuid) == g_identity.xuid),
        Read<uint8_t>(kAuth + 0x2D0), int(owns), Read<uint8_t>(kContentFinished),
        Read<uint8_t>(kOfflineStatsFetched), Read<uint32_t>(kLastBnetError));
}
}
