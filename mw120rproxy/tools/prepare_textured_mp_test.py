...
def decode_iwi(data):
    ...
    size = width*height*{1:4,2:3,3:2,4:1,5:1}[fmt] if fmt<=5 else ((width+3)//4) * ((height+3)//4) * block_bytes
    ...
    if fmt<=5:
        ...
        for face in range(faces):
            ...
            if fmt in (1,2):
                ...
                image=Image.frombytes(mode,(width,height),payload,'raw',rawmode).convert('RGBA')
            elif fmt==5:
                image=Image.new('RGBA',(width,height),(255,255,255,255));image.putalpha(Image.frombytes('L',(width,height),payload))
            else:image=Image.frombytes('LA' if fmt==3 else 'L',(width,height),payload).convert('RGBA')
            result.append(image)
        return result
...
def sky_surfaces():
    ...
    mappings = (lambda u,v:(1,-v,-u), lambda u,v:(-1,-v,u),
                lambda u,v:(u,1,v), lambda u,v:(u,-1,-v),
                lambda u,v:(u,-v,1), lambda u,v:(-u,-v,-1))
    result = []
    for face, mapping in enumerate(mappings):
        vertices = [{'position':[float(x*32768) for x in mapping(u*2-1,v*2-1)],
                     'uv':[u,v], 'normal':3489135616, 'lightmapUV':[face+2,0]}
