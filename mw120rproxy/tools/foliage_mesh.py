...
def mask_rectangles(image, size=128, threshold=128):
    w, h = image.size
    factor = min(1, size / max(w, h))
    w = max(1, round(w * factor))
    h = max(1, round(h * factor))
    ...
    for y in range(h):
        ...
        while x < w:
            if pixels[y * w + x] < threshold:
                ...
                continue
            start = x
            while x < w and pixels[y * w + x] >= threshold:
                x += 1
            runs.append((start, x))
        next_active = {}
        for run in runs:
            next_active[run] = (active.pop(run, (y, y))[0], y + 1)
...
def reverse_normal(packed):
    n = [-x for x in unpack_normal(packed)]
    axis = [0, 0, 1] if abs(n[2]) < 0.9 else [0, 1, 0]
    t = [
...
def collapsed_uv_polygons(tri, mask):
    ...
    lo = min(tri, key=lambda v: v["uv"][axis])
    hi = max(tri, key=lambda v: v["uv"][axis])
    if hi["uv"][axis] - lo["uv"][axis] < 1e-10:
        return [tri] if mask.covers(shapely.Point(*(x % 1 for x in lo["uv"]))) else []
    line = shapely.LineString([lo["uv"], hi["uv"]])
    ranges = []
    tx0, ty0 = map(math.floor, line.bounds[:2])
    tx1, ty1 = map(math.ceil, line.bounds[2:])
    tx1 = max(tx0 + 1, tx1)
    ty1 = max(ty0 + 1, ty1)
    if (tx1 - tx0) * (ty1 - ty0) > 64:
        raise ValueError("Alpha-tested triangle repeats its texture more than 64 times; split it")

    def segments(g):
        if g.geom_type == "LineString" and not g.is_empty:
            values = [p[axis] for p in g.coords]
    ...
    for ty in range(ty0, ty1):
        for tx in range(tx0, tx1):
            segments(line.intersection(translate(mask, xoff=tx, yoff=ty)))
    ...
    def clip(poly, bound, greater):
        ...
        if not poly:
            return out
        a = poly[-1]
        inside = lambda v: v["uv"][axis] >= bound if greater else v["uv"][axis] <= bound
        last = inside(a)
        for b in poly:
            current = inside(b)
            if current != last:
                ...
                out.append(
                    {
                        "position": [x + (y - x) * t for x, y in zip(a["position"], b["position"])],
                        "uv": [x + (y - x) * t for x, y in zip(a["uv"], b["uv"])],
                        "normal": (a if t < 0.5 else b)["normal"],
                    }
                )
            if current:
                out.append(b)
...
def cutout(surface, mask, two_sided=True, triangle_budget=250000):
    ...
    def flush():
        ...
        if indices:
            output.append({**surface, "vertices": vertices, "indices": indices})
        vertices = []
        indices = []
    ...
    for start in range(0, len(surface["indices"]), 3):
        ...
        if sum(x * x for x in cross) < 1e-12:
            continue
        us = [v["uv"][0] for v in tri]
        vs = [v["uv"][1] for v in tri]
        area = (us[1] - us[0]) * (vs[2] - vs[0]) - (vs[1] - vs[0]) * (us[2] - us[0])
        if abs(area) < 1e-12:
            for poly in collapsed_uv_polygons(tri, mask):
                for i in range(1, len(poly) - 1):
                    batch = [poly[0], poly[i], poly[i + 1]]
                    count = 2 if two_sided else 1
                    if emitted + count > triangle_budget:
                        raise ValueError("Foliage geometry budget exceeded")
                    if len(vertices) + 3 * count > 60000:
                        flush()
                    for back in range(count):
                        for v in (list(reversed(batch)) if back else batch):
                            indices.append(len(vertices))
                            vertices.append(
                                {**v, "normal": reverse_normal(v["normal"])} if back else v
                            )
                    emitted += count
            continue
        tx0, tx1, ty0, ty1 = (
        ...
        if (tx1 - tx0) * (ty1 - ty0) > 64:
            raise ValueError(
                "Alpha-tested triangle repeats its texture more than 64 times; split it"
            )
        ...
        for ty in range(ty0, ty1):
            for tx in range(tx0, tx1):
                ...
                for piece in triangles.geoms:
                    ...
                    if emitted + count > triangle_budget:
                        raise ValueError(
                            "Foliage geometry budget exceeded; use a lower mask size or simpler foliage"
                        )
                    if len(vertices) + 3 * count > 60000:
                        flush()
                    batch = []
                    for u, v in pts:
                        ...
                        vertex = {
                            "position": [
                                sum(weights[j] * tri[j]["position"][k] for j in range(3))
                                for k in range(3)
                            ],
                            "uv": [u, v],
                            "normal": tri[max(range(3), key=lambda j: weights[j])]["normal"],
                        }
                        batch.append(vertex)
                    for back in range(count):
