...
def quaternion(t,b,n):
    ...
    n=norm(n);t=norm([t[i]-dot(t,n)*n[i] for i in range(3)])
    cb=cross(n,t);sign=1 if dot(cb,b)>=0 else -1
    ...
    if trace>0:
        ...
        s=math.sqrt(1+m[i][i]-m[j][j]-m[k][k])*2;q=[0.0]*4
...
def pack(q,sign):
    ...
    largest=max(range(4),key=lambda i:(abs(q[i]),i));scale=math.sqrt(2)*(1 if q[largest]>=0 else -1)
    xyz=[q[i]*scale for i in range(4) if i!=largest]
    ints=[max(0,min((1<<bits)-1,int((v+1)*((1<<bits)-1)/2))) for v,bits in zip(xyz,(10,10,9))]
    return ints[0]|ints[1]<<10|ints[2]<<20|(sign<0)<<29|largest<<30

def unpack_normal(packed):
    largest=packed>>30
    values=[((packed&1023)/1023*2-1)/math.sqrt(2),
            (((packed>>10)&1023)/1023*2-1)/math.sqrt(2),
            (((packed>>20)&511)/511*2-1)/math.sqrt(2)]
    q=values[:];q.insert(largest,math.sqrt(max(0,1-sum(v*v for v in values))))
    x,y,z,w=q
