...
def geometry(lumps):
    ...
    for label, ids in [('layered', (9, 10, 11)), ('simple', (47, 48, 49))]:
        ...
        for v in records(lumps[ids[1]], '<6f4B10f'):
            vertices.append(dict(position=finite(v[:3]), normal=finite(v[3:6]),
                                 rgba=list(v[6:10]), uv=finite(v[10:12]),
                                 lightmap_uv=finite(v[12:14]), remaining_floats=finite(v[14:])))
        indices = [v[0] for v in records(lumps[ids[2]], '<H')]
        soups = []
        for raw, first_vertex, num_vertices, num_indices, first_index in records(lumps[ids[0]], '<12sIHHI'):
            if first_vertex + num_vertices > len(vertices) or first_index + num_indices > len(indices) or num_indices % 3:
            ...
            soups.append(dict(source_metadata_hex=raw.hex(), first_vertex=first_vertex,
                              num_vertices=num_vertices, first_index=first_index,
                              triangles=[[first_vertex + j for j in local[i:i+3]] for i in range(0, len(local), 3)]))
    ...
    result['triangle_collision'] = dict(vertices=collision_vertices, triangles=collision_triangles,
        includes_brush_collision=False, havok_cooked=False)
...
def main():
    ...
    map_id = args.map
    files = [f'map_source/{map_id}.map', f'raw/maps/mp/{map_id}.d3dbsp',
             f'raw/maps/mp/{map_id}.gsc', f'zone/english/{map_id}.ff', f'zone/english/{map_id}_load.ff']
    sources = []
    for relative in files:
        path = root / relative
        data = path.read_bytes()
        sources.append(dict(path=str(path), relative_path=relative, size=len(data), sha256=digest(data)))
    ...
    report = dict(schema=1, map=map_id, sources=sources, lump_count=len(entries), lumps=entries,
        entity_classes=dict(sorted(classes.items())), entity_count=sum(classes.values()),
        geometry={name: dict(vertices=len(geo[name]['vertices']), surfaces=len(geo[name]['surfaces']),
                  triangles=sum(len(s['triangles']) for s in geo[name]['surfaces']))
                  for name in ('layered', 'simple') if name in geo},
        triangle_collision=dict(vertices=len(geo['triangle_collision']['vertices']),
                                triangles=len(geo['triangle_collision']['triangles']), includes_brush_collision=False),
        world_bounds=geo['world_bounds'], native_iw8_assets_emitted=False, game_launched=False,
        remaining=['Replay fastfile acceptance and asset layouts', 'IW8 world rendering and material conversion',
                   'Havok collision including brush solids', 'IW8 spawn classes and map scripts', 'owner live match'])
