...
def create(folder,stem,mapid,kind='glass'):
    if kind not in ('glass','foliage'):raise ValueError('Invalid material kind')
    path=folder/(stem+'.techset.json');ts=json.loads(path.read_text())
    ...
    lit=copy.deepcopy(ts['techniques'][-1]);header=bytearray.fromhex(lit['header'])
    ...
    other=(other & ~(0xE00|3)) | (0xC00 if kind=='glass' else 0xE00)  # depth test, no depth writes, two-sided
    ...
    for off in range(0,len(states),16):states[off:off+8]=hashlib.sha256(bytes(header)+bytes([off])+lit['shaders'][3].encode()).digest()[:8]
    lit['states']=states.hex();ts['techniques']=[lit]
    th=bytearray.fromhex(ts['header']);th[24:56]=bytes(32);struct.pack_into('<Q',th,24,1<<34)
    ts['header']=th.hex()
    needed={f'{14+i}:{n}' for i,n in enumerate(lit['shaders']) if n}
    ts['shaders']={k:v for k,v in ts['shaders'].items() if k in needed}
    definition=stem+'.'+kind+'.material.json';techfile=stem+'.'+kind+'.techset.json'
    material['techset']=ts['name'];material['techsetDefinition']=techfile
    (folder/definition).write_text(json.dumps(material,indent=2)+'\n')
    (folder/techfile).write_text(json.dumps(ts,indent=2)+'\n')
    return {'schema':1,'material':'w/mw120r_'+mapid+'_'+kind,'materialDefinition':definition}
