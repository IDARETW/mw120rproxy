...
def extract(args):
    ...
    if output.exists() and any(output.iterdir()):
        raise ValueError("Use a fresh output directory to preserve existing dumps")
    ...
    run(
        [
            ...
            "OBJ",
            "--search-path",
            str(COD4 / "main") + ";" + str(COD4 / "raw") + ";" + str(source),
            "-o",
            output,
            ff,
        ],
        ...
        with zipfile.ZipFile(archive) as z:
            for entry in z.infolist():
                ...
                record["images"].append(
                    {"name": entry.filename, "sha256": hashlib.sha256(data).hexdigest()}
                )
    ...
    write_json(
        output / "extraction_report.json",
        {
            "map": args.map,
            "source": str(ff),
            "source_sha256": hashlib.sha256(ff.read_bytes()).hexdigest(),
            "unlinker": str(oat),
            "unlinker_sha256": hashlib.sha256(oat.read_bytes()).hexdigest(),
            "archives": archives,
