...
def surface_type(source, name):
    ...
    if p.exists():
        bits = json.loads(p.read_text()).get("surfaceTypeBits", 0)
    else:
        ...
        p = Path(r"E:/CoD4/game_build/Call of Duty 4 Modern Warfare/raw/materials") / name.lstrip(
            ","
        )
...
def encode(surfaces, types):
    ...
    for s in surfaces:
        ...
        for i in range(0, len(s["indices"]), 3):
            ...
            kind = int(vs[0]["lightmapUV"][1]) % 4
            if tile >= len(types) or kind in (1, 2, 3):
                continue  # sky, glass, cutout overlays
            a, b, c = [v["position"] for v in vs]
            u = [b[k] - a[k] for k in range(3)]
            v = [c[k] - a[k] for k in range(3)]
            n = [u[1] * v[2] - u[2] * v[1], u[2] * v[0] - u[0] * v[2], u[0] * v[1] - u[1] * v[0]]
            # Replay BSP winding is clockwise. Retain only upward walkable faces.
            if -n[2] < 0.35 * math.sqrt(sum(x * x for x in n)) or abs(n[2]) < 0.001:
                continue
            t = types[tile]
            records.append(struct.pack("<9fI", *(a + b + c), t))
            counts[t] += 1
    if len(records) > 600000:
        raise ValueError("Footstep triangle budget exceeded")
    return b"MWRSTEP1" + struct.pack("<I", len(records)) + b"".join(records), dict(counts)


def generate(package, source, report, render):
    names = report["color_images"]
    types = {}
    for name, m in report["materials"].items():
        if "image" not in m:
            continue
        t = surface_type(source, name)
        if m["image"] not in types or t != 5:
            types[m["image"]] = t
