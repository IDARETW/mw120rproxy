...
def configure(root, with_wavelet=False):
    ...
    if actual != PIN:
        raise ValueError(f"Expected OpenAssetTools v0.33.0 {PIN}, found {actual}")
    directory = root / "src/ObjWriting/Game/IW3"
    writer = directory / "ObjWriterIW3.cpp"
    text = writer.read_text()
    if '#include "ReplayMapDumpers.h"' not in text:
        anchor = '#include "ObjWriterIW3.h"'
        if text.count(anchor) != 1:
            raise ValueError("Missing ObjWriter include anchor")
        text = text.replace(anchor, anchor + '\n#include "ReplayMapDumpers.h"')
    registrations = (
        ("AssetDumperClipMap", "replay_export::Collision<IW3::AssetClipMap>"),
        ("AssetDumperGfxWorld", "replay_export::World"),
    )
    for old, new in registrations:
        if f"std::make_unique<{new}>()" in text:
            continue
        anchor = f"// REGISTER_DUMPER({old})"
        if text.count(anchor) != 1:
            raise ValueError(f"Missing registration anchor {anchor}")
    ...
    if pvs not in text:
        anchor = (
            "RegisterAssetDumper(std::make_unique<replay_export::Collision<IW3::AssetClipMap>>());"
        )
        text = text.replace(anchor, anchor + "\n    " + pvs)
    shutil.copyfile(
        Path(__file__).with_name("ReplayMapDumpers.h"), directory / "ReplayMapDumpers.h"
    )
        ...
        for name in ("IwiLoader.cpp", "IwiWaveletDecoder.cpp", "IwiWaveletDecoder.h"):
            ...
            if destination.exists() and destination.read_bytes() != replacement:
                original = subprocess.run(
                    ["git", "show", PIN + ":" + path], cwd=root, capture_output=True
                )
                if original.returncode or destination.read_bytes().replace(
                    b"\r\n", b"\n"
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    configure(a.checkout, a.with_wavelet)
