...
struct Input {float4 position:SV_POSITION;float4 uv:TEXCOORDS0;float2 lightmapUV:LMAPCOORDS0;float3 normal:NORMAL0;float4 tangent:TANGENT0;};
Texture2D<float4> sourceAtlas:register(t1);
SamplerState colorSampler:register(s3);
cbuffer ReplayLighting:register(b7){float4 replayLighting[43];};
float4 main(Input input):SV_TARGET0 {
    ...
    float3 light=lerp(float3(.22,.23,.24),float3(.38,.41,.44),hemi)+ndotl*SUN_COLOR*.65;
    if(flags>=4){
        float4 lm=sourceAtlas.SampleLevel(colorSampler,saturate(frac(input.lightmapUV)*4-1),0);
        ...
        float3 baked=lerp(lm.rgb*12.92,1.055*pow(max(lm.rgb,0),1.0/2.4)-.055,step(.0031308,lm.rgb));
