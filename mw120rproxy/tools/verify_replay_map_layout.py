...
def verify(game, package, map_id):
    if not re.fullmatch(r'mp_[a-z0-9_]{1,60}',map_id):raise ValueError('Invalid map id')
    ...
    def owned_techset(name):
        return re.fullmatch(b'tw/mw120r_'+map_id.encode('ascii')+b'_(?:(?:glass|foliage)_)?[0-9a-f]{12}',name) is not None
    ...
    if manifest.get('ladders') or (package/'ladders.bin').exists():
        ...
        if manifest.get('ladders') not in ('faces-v1','faces-v2'):raise ValueError('Unsupported ladder manifest')
    ...
    if manifest.get('glass') or (package/'glass.bin').exists():
        ...
        if manifest.get('glass') not in ('panes-v1','panes-v2'):raise ValueError('Unsupported glass manifest')
        validate_glass((package/'glass.bin').read_bytes())
    if manifest.get('collision') not in (None,'boxes-v1','convex-v2'):raise ValueError('Unsupported collision manifest')
    ...
    sizes = {t: struct.unpack('<I', pe.get_data(0x2458160 + 4*t, 4))[0]
             for t in (11, 14, 17, 18, 19, 23, 24, 25, 29, 31, 61)}
    if sizes != {11: 120, 14:40, 17:40, 18:64, 19:232, 23: 0xF8, 24: 0xA8, 25: 0x10, 29: 0x408, 31: 0x4590, 61: 0x20}:
    ...
    for prefix, types in (('srv_', [29, 23, 24] + [61]*41), ('', [31, 25]),
                          ('eng_', []), ('ww_', []), ('techsets_', [])):
        ...
        def name(expected=expected_name):
            ...
            if end < 0 or body[pos:end] != expected:
                raise ValueError(f'{path.name}: wrong asset string at {pos:#x}; layout is desynchronized')
            virtual += end+1-pos
            pos = end+1
        def q(buf, off):
        ...
        def read_material(material, allow_definition=False):
            ...
            if reference not in (b',$default',b',mo/white_3d',b',w/mw120r_test',b'w/mw120r_test',owned_material,b','+owned_material,glass_material,b','+glass_material,foliage_material,b','+foliage_material):
                raise ValueError('Unrecognized material reference')
            name(reference)
            if reference.startswith(b','):
                if any(material[8:]):raise ValueError('Reference must contain only its name')
            else:
                if not allow_definition or reference not in (b'w/mw120r_test',owned_material,glass_material,foliage_material) or material[28:32]!=bytes([3,4,1,0]):
                ...
                for off in (0x40,0x48,0x50,0x60,0x68):
                    if q(material,off)!=2**64-2:raise ValueError('Missing material array')
                techset=take(64)
                if q(techset,0)!=2**64-2 or any(techset[8:]):raise ValueError('Invalid techset reference')
                ts_name=body[pos:body.index(b'\0',pos)]
                if ts_name not in (b',w/lit_3_lit_rpl_ta1_804000_1002000000000010_0_1_1_0_0_13810015b_0_0_1_0_0',b',w/mw120r_graybox_v1',b',tw/mw120r_graybox_v1') and not (ts_name.startswith(b',') and owned_techset(ts_name[1:])):raise ValueError('Unexpected world technique reference')
                name(ts_name)
                textures=vtake(48,7)
                for i in range(3):
                    if q(textures,i*16+8)!=2**64-2:raise ValueError('Missing image reference')
                    im=take(0xE8)
                    if q(im,0)!=2**64-2 or any(im[8:]):raise ValueError('Invalid neutral image')
                    image=body[pos:body.index(b'\0',pos)]
                    if image not in (b',$gray',b',$identitynormalmap',b',$black') and image not in {b','+x for x in image_names}:
                        raise ValueError('Missing ordered material image dependency')
                    name(image)
                vtake(80,15)
                indices=vtake(195,0)
                if any(x not in (0,255) for x in indices):raise ValueError('Constant buffer index outside table')
                cb=vtake(0x110,15)
                if any(cb[0x30:]):raise ValueError('Serialized GPU constant buffer handles')
                for k in range(4):
                    length=u(cb,4*k)
                    if length>65536 or q(cb,16+8*k)!=(2**64-2 if length else 0):raise ValueError('Invalid shader constant stage')
                    if length:vtake(length,15)
            return reference
        def mapents():
            nonlocal virtual
            me = take(sizes[29])
            ...
            if count:
                ...
                for ent in re.findall(rb'\{([^{}]*)\}',ents):
                    ...
                    if classname.startswith(b'mp_') and b'_spawn' in classname:expected.append(fields)
                if len(expected)!=count:raise ValueError('Compiled spawns do not cover entity source')
                for i,fields in enumerate(expected):
                    index,pad,n,t,s,*vectors=struct.unpack_from('<HHIII6f',records,i*40)
                    if index!=i or pad or min(n,t,s)==0 or max(n,t,s)>=len(script_strings):
                        raise ValueError('Invalid compiled spawn record or script-string index')
                    if [script_strings[x] for x in (n,t,s)] != [fields.get(k,b'') for k in (b'212',b'1070',b'845')]:
                        raise ValueError('Spawn script-string references differ from source')
                    expected_vec=[float(v) for k in (b'709',b'80') for v in fields.get(k,b'0 0 0').split()]
            ...
            for off in (0x1C8,0x1D0):
                if q(me,off):
                    ...
                    if q(me,off)!=2**64-2:raise ValueError('Invalid population pointer')
                    if any(population):
                        if q(population,0)!=2**64-2 or q(population,8)!=2**64-2 or u(population,0x28)!=1 or u(population,0x2C):
                        ...
                        if any(vtake(8,3)):raise ValueError('Expected unsplit leaf')
                        if struct.unpack('<6f',vtake(24,3))!=(-100000.,)*3+(100000.,)*3:
                            raise ValueError('Invalid spatial partition extents')
                        if vtake(4,3)!=b'\xff'*4:raise ValueError('Missing empty spatial bucket sentinel')
            if q(me,0x1C8) and u(me,0x1E8)!=0xFFFFFFFF:
                raise ValueError('Missing empty non-spatial entity sentinel')
            if q(me,0x2C0):
                tree=vtake(24,7)
                if q(me,0x2C0)!=2**64-2 or q(tree,0)!=2**64-2 or q(tree,8)!=2**64-2 or u(tree,16)!=1:
                    raise ValueError('Invalid scriptable spatial tree')
                if any(vtake(8,3)) or struct.unpack('<6f',vtake(24,3))!=(-100000.,)*3+(100000.,)*3:
                    raise ValueError('Invalid scriptable spatial leaf')

        root = take(32)
        if prefix=='' and u(root,16)==3:types=[11,31,25]
        if prefix=='' and u(root,16)==12:types=[14]*4+[17]*4+[18,11,31,25]
        if prefix=='' and u(root,16)==10:types=[14]*3+[17]*3+[18,11,31,25]
        image_candidate=prefix=='' and 10<=u(root,16)<=18
        if image_candidate:types=[0]*u(root,16)
        if u(root, 16) != len(types) or u(root, 0)>65536:
            raise ValueError('Unexpected asset or script-string counts')
        script_strings=[]
        if u(root,0):
            if q(root,8)!=2**64-2:raise ValueError('Missing inline script-string list')
            pointers=struct.unpack(f'<{u(root,0)}Q',vtake(u(root,0)*8,7))
            for i,pointer in enumerate(pointers):
                if not pointer:
                    script_strings.append(b'');continue
                if pointer!=2**64-2:raise ValueError('Invalid script-string pointer')
                end=body.find(b'\0',pos)
                if end<0:raise ValueError('Unterminated script string')
                value=body[pos:end];script_strings.append(value);name(value)
            if script_strings[0]!=b'':raise ValueError('Missing null script-string index zero')
        ...
        if image_candidate:
            ...
            if types not in ([19]*image_count+[14]*3+[17]*3+[18,11,31,25],
                [19]*image_count+[14]*3+[17]*3+[18,11,18,11,31,25], [19]*image_count+[14]*3+[17]*3+[18,11,18,11,18,11,31,25], [14]*4+[17]*4+[18,11,31,25]):
        ...
        for asset_type in types:
            ...
            if asset_type == 19:
                ...
                levels=asset[0x30]
                if not 1<=levels<=max(width,height).bit_length():raise ValueError('Invalid resident mip count')
                ...
                if asset!=expected:raise ValueError('Invalid resident image metadata or serialized runtime pointer')
                # Native E2FFF0 consumes these bytes from stream 1, not virtual.
                pixels=take(length)
                if blocks[1]<len(pixels)+232:raise ValueError('Resident image exceeds temporary stream reservation')
                image_names.add(sn)
                continue
            if asset_type in (14,17):
                sn=body[pos:body.index(b'\0',pos)];name(sn)
                if len(sn)>128 or not sn:raise ValueError('Invalid shader name')
                shader_names.add((asset_type,sn))
                if q(asset,8):
                    if q(asset,8)!=2**64-2:raise ValueError('Invalid debug name tag')
                    name(body[pos:body.index(b'\0',pos)])
                length=u(asset,32)
                if q(asset,16) or length>65536 or q(asset,24)!=(2**64-2 if length else 0):raise ValueError('Invalid serialized shader program')
                if length and not vtake(length,3).startswith(b'DXBC'):raise ValueError('Shader program is not DXBC')
                continue
            if asset_type == 18:
                ts_name=body[pos:body.index(b'\0',pos)]
                if ts_name not in (b'w/lit_3_lit_rpl_ta1_804000_1002000000000010_0_1_1_0_0_13810015b_0_0_1_0_0',b'w/mw120r_graybox_v1',b'tw/mw120r_graybox_v1') and not owned_techset(ts_name):raise ValueError('Unexpected world technique definition')
                name(ts_name)
                if q(asset,24) not in (0x418000009,0x418000001,1<<34) or any(asset[32:56]) or q(asset,56)!=2**64-2:raise ValueError('Unexpected world technique mask')
                count=q(asset,24).bit_count()
                if vtake(count*8,7)!=struct.pack('<'+str(count)+'Q',*([2**64-2]*count)):raise ValueError('Missing technique pointers')
                for expected_type in ([34] if count==1 else [0,27,28,34] if count==4 else [0,3,27,28,34]):
                    te=vtake(184,7)
                    if q(te,0)!=2**64-2 or u(te,8)!=expected_type or any(te[0x30:0x50]):raise ValueError('Invalid technique header/runtime pointers')
                    if count==4 and te[0x9C] not in (32,34,35):raise ValueError('Static BSP has incompatible shader layout')
                    if count==1 and (te[0x9C]!=35 or (q(te,0xA0),q(te,0xA8))!=((0xE00,0) if b'_foliage_' in ts_name else (0xC00,0x28054)) or not any(k in ts_name for k in (b'_foliage_',b'_glass_'))):
                        raise ValueError('Invalid transparent BSP blend/depth state')
                    tn=body[pos:body.index(b'\0',pos)];name(tn)
                    if not tn.startswith(b'TECHNIQUE_'):raise ValueError('Invalid technique name')
                    for off in (0x28,0x50,0x88,0xB0):
                        if q(te,off)!=2**64-2:raise ValueError('Missing technique array')
                    states=vtake(te[0x83]*te[0xE]*16,7)
                    if any(any(states[o:o+8]) for o in range(8,len(states),16)):raise ValueError('Serialized pipeline handle')
                    vtake(16,0)
                    for typ,off in ((14,0x58),(15,0x60),(16,0x68),(17,0x70)):
                        if not q(te,off):continue
                        if q(te,off)!=2**64-2:raise ValueError('Invalid shader pointer tag')
                        sh=take(40)
                        if q(sh,0)!=2**64-2 or any(sh[8:]):raise ValueError('Shader reference has data')
                        sn=body[pos:body.index(b'\0',pos)];name(sn)
                        if not sn.startswith(b',') or (typ,sn[1:]) not in shader_names:raise ValueError('Missing ordered shader dependency')
                    vtake(te[0x83]*5,0);vtake(sum(te[0x78:0x7C])*6,1)
                continue
            if asset_type == 61:
                name(f'ncs_{tags[ncs_index]}_level'.encode())
                if u(asset, 8) != ncs_index or u(asset, 12) != 2 or u(asset, 16) or u(asset, 20) or q(asset, 24):
            ...
            if asset_type == 23:
                ...
                if q(asset,0x20):
                    ...
                    stage=vtake(40,7)
                    if q(stage,0)!=2**64-2 or stage[0x16] not in (0,1) or any(stage[8:0x16]+stage[0x17:]):
                ...
                if lights==2:
                    ...
                    if (sun[1]!=1 or q(sun,0x98) or struct.unpack_from('<f',sun,0x10)[0]<=0
                ...
                name()
                if u(asset,0x18) not in (1,2) or u(asset,0x14)!=u(asset,0x18)-1 or q(asset, 0x3D68)!=2**64-2 or blocks[4]<u(asset,0x18)*152+4:
                ...
                if u(asset,0x3FA0)!=1 or q(asset,0x41C0)!=2**64-2 or q(asset,0x3EF8)!=2**64-2:
                ...
                if (u(asset, 0x90) != 1 or struct.unpack_from('<H', asset, 0xA0)[0] != 1
                        or q(asset, 0xA8) != 2**64-2 or q(asset, 0xB8) != 2**64-2
                        or q(asset, 0x3EF0) != 2**64-2 or q(asset, 0x3F30)):
                ...
                count=u(asset,0xC8)
                if count>4096:raise ValueError('Excessive mesh surface count')
                words=(count+31)//32
                surface_records=[]
                gpu_records=[]
                if count:
                    ...
                    if any(q(asset,o)!=2**64-2 for o in (0xF0,0xF8,0x100,0x108,0x41E0,0x41F8)):
                    ...
                    for sf in surface_records:
                        if q(sf,16)!=2**64-2:raise ValueError('Missing surface material')
                        material_refs.append(read_material(take(120)))
                    transparent=sum(r in (b','+glass_material,b','+foliage_material) for r in material_refs)
                    opaque=count-transparent
                    if any(r in (b','+glass_material,b','+foliage_material) for r in material_refs[:opaque]) or any(r not in (b','+glass_material,b','+foliage_material) for r in material_refs[opaque:]):raise ValueError('Cutout/glass must follow opaque surfaces')
                    if [u(asset,o) for o in range(0xD0,0xF0,4)]!=[0,opaque,opaque,opaque,opaque,count,count,count]:
                        raise ValueError('Native surface sort ranges disagree with materials')
                    vtake(count*56,3)
                    if any(vtake(count*16,7)):raise ValueError('Expected runtime-sorted draw surfaces')
                ...
                if q(transient, 0) != 2**64-2 or u(transient,0xD8)!=1 or q(transient,0xE0)!=2**64-2 or q(transient,0xE8)!=2**64-2:
                    raise ValueError('Unexpected resident render zone')
                name()
                if count:
                    pos_size,aux_size,index_count=u(transient,0x10),u(transient,0x14),u(transient,0xA8)
                    if any(q(transient,o)!=2**64-2 for o in (0x18,0x20,0xB0)):
                        raise ValueError('Missing geometry buffers')
                    positions=vtake(pos_size,3);aux=vtake(aux_size,3);indices=vtake(index_count*2,3)
                    for i,(sf,gpu) in enumerate(zip(surface_records,gpu_records)):
                        ...
                        if not verts or not tris or po+verts*12>len(positions) or base_index+tris*3>index_count:
                        ...
                        if max(ix)>=verts:raise ValueError('Triangle index exceeds vertex count')
                        for off,stride in ((12,4),(16,8),(24,8)):
                            if u(gpu,off)+verts*stride>len(aux):raise ValueError('Auxiliary attribute exceeds buffer')
                if take(4)!=b'\x01\0\0\0' or take(8)!=b'\xfe'+b'\xff'*7 or take(4)!=b'\x01\0\0\0':
                    raise ValueError('Resident cell tree framing differs')
                virtual=((virtual+3)&~3)+4
                virtual=((virtual+7)&~7)+8
                tree=take(48)
                virtual=((virtual+7)&~7)+48
                if u(tree,24)!=count or any(tree[28:]):raise ValueError('Cell tree does not cover mesh surfaces')
                preload_size = ((sizes[31]+31) & ~31)+32
                model=take(96)
                if u(asset, 0x3DF0)!=1 or q(asset, 0x3DF8)!=2**64-2 or u(model,0x58)!=count or any(model[:0x58]) or any(model[0x5C:]):
        ...
        if blocks[5] < virtual:
            raise ValueError(f'{path.name}: VIRTUAL reservation {blocks[5]} < native requirement {virtual}')
        report['zones'].append({'file': path.name, 'body_bytes_consumed': pos,
                                'virtual_bytes_required': virtual, 'virtual_bytes_reserved': blocks[5],
