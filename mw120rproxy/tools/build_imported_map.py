...
def build(args):
    ...
    out = REPO / f"custom_map_sources/{mapid}/builds" / datetime.now().strftime("%Y%m%d-%H%M%S-%f")
    ...
    if len(names) > 250:
        raise ValueError(f"{len(names)} images exceeds 250-image atlas budget")
    columns = 1
    while columns * columns < len(names) + 6:
        columns *= 2
    ...
    for i, name in enumerate(names):
        faces = image(name)
        if len(faces) != 1:
            raise ValueError(f"{name}: expected 2D color image")
        atlas.paste(
            faces[0].resize((cell, cell), Image.Resampling.LANCZOS),
    ...
    image_name = f"mw120r/{mapid}_" + hashlib.sha256(pixels).hexdigest()[:16]
    material["textures"][0]["image"] = image_name
    material["techsetDefinition"] = stem + ".techset.json"
    material["imageDefinitions"] = [
        {"name": image_name, "width": 4096, "height": 4096, "rgba8": mapid + "_atlas.rgba"}
    ]
    ...
    gc.collect()
    run(
        [converter, "fromdump", out / "dump", mapid, "-o", package, "--stored"],
        REPO,
        out / "convert.log",
    )
