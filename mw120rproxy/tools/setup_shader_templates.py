...
def capture(pid, game):
    ...
    try:
        ...
        if not k.QueryFullProcessImageNameW(handle,0,name,C.byref(n)):raise C.WinError(C.get_last_error())
        if Path(name.value).resolve()!=expected:raise ValueError('PID belongs to another executable')
        modules=(C.c_void_p*1024)();needed=W.DWORD()
        if not ps.EnumProcessModulesEx(handle,modules,C.sizeof(modules),C.byref(needed),3):raise C.WinError(C.get_last_error())
        base=modules[0]
        def read(ptr,size):
            if not ptr or not 0<size<=1024*1024:raise ValueError('Invalid bounded shader read')
            buf=C.create_string_buffer(size);got=C.c_size_t()
            if not k.ReadProcessMemory(handle,ptr,buf,size,C.byref(got)) or got.value!=size:raise C.WinError(C.get_last_error())
            return buf.raw
        def q(buf,off=0):return struct.unpack_from('<Q',buf,off)[0]
        def u(buf,off=0):return struct.unpack_from('<I',buf,off)[0]
        def asset(kind,name):
            value=kind
            for byte in name.encode('ascii'):value=(value*31+byte)&0xffffffff
            table=base+0xC815C60
            index=u(read(table+0x6E0F08+(value%0x57800)*4,4));visited=set()
            for _ in range(1024):
                if not index:return 0
                if index>=0x6E0F08//20 or index in visited:raise ValueError('Invalid/cyclic asset chain')
                visited.add(index);entry=read(table+index*20,20);ptr=q(entry)
                if entry[17]==kind and read(q(read(ptr,8)),len(name)+1)==name.encode()+b'\0':return ptr
                index=u(entry,8)
        ...
        for shader in graph['shaders'].values():
            ...
            if size:
                ...
                if not ptr:raise ValueError('Shader is not resident; load stock Shipment first: '+shader['name'])
                header=read(ptr,40)
                if u(header,32)!=size:raise ValueError('Unexpected native shader size')
                program=read(q(header,24),size)
            else:program=b''
            validate_program(shader,program)
            shader.pop('program_sha256');shader['program']=program.hex()
        ...
        if shaderfile.exists() or material.exists():raise FileExistsError('Templates already exist; preserve or move them before running setup again')
        shaderfile.parent.mkdir(parents=True,exist_ok=True);material.parent.mkdir(parents=True,exist_ok=True)
...
if __name__=='__main__':
    ...
    a=p.parse_args();capture(a.pid,a.game)
