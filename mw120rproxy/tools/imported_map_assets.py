...
def packed_normal(n, t=None, sign=1):
    ...
    if t is None or dot(cross(t, n), cross(t, n)) < 1e-10:
        t = cross([0, 0, 1] if abs(n[2]) < 0.9 else [0, 1, 0], n)
...
def safe_asset(root, folder, name, extension):
    ...
    if not p.is_relative_to(root.resolve()):
        raise ValueError("Asset path escapes dump")
    return p
...
def read_material(root, name):
    ...
    if tech == "shadowcaster":
        return {"skip": "shadow-only helper", "techset": tech}
    blended = bool(re.search(r"(^|_)b\d", tech))
    if re.search(r"(^|_)sky($|_)", tech):
        return {"skip": "sky surface", "techset": tech}
    if not color:
        raise ValueError(f"{name}: no color map in {tech}")
    alpha = bool(
        re.search(r"(^|_)[at]\d", tech)
        or "alphatest" in tech
        or any(
            state.get("alphaTest") in ("gt0", "lt128", "ge128") for state in j.get("stateBits", [])
        )
    )
    ...
    visible = next(
        (s for s in j.get("stateBits", []) if s.get("colorWriteRgb") and not s.get("polymodeLine")),
        {},
    )
...
def read_obj(root, name):
    ...
    for line in path.read_text().splitlines():
        ...
        if key in ("v", "vn"):
            ...
            (positions if key == "v" else normals).append([x, -z, y])
        elif key == "vt":
            uvs.append([float(values[0]), 1 - float(values[1])])
        elif key == "usemtl":
            current = {"material": values[0], "vertices": [], "indices": []}
            surfaces.append(current)
            mapping = {}
        elif key == "f":
            if current is None or len(values) != 3:
                raise ValueError(f"{name}: expected material-bound triangles")
            tri = []
            for value in values:
                vi, ti, ni = map(int, value.split("/"))
                if not (
                    1 <= vi <= len(positions) and 1 <= ti <= len(uvs) and 1 <= ni <= len(normals)
                ):
...
def brush_hull(brush, index):
    ...
    if not planes:
        ...
        return {
            "vertices": points,
            "source": f"compiled brush {index}",
            "contents": brush["contents"],
        }
    for k in range(3):
        n = [0.0, 0.0, 0.0]
        n[k] = 1.0
        planes.append(n + [hi[k]])
        n = [0.0, 0.0, 0.0]
        n[k] = -1.0
        planes.append(n + [-lo[k]])
    if len(planes) > 70:
        raise ValueError(f"Brush {index}: too many planes")
    points = []
    for a, b, c in combinations(planes, 3):
        ...
        if all(dot(n[:3], p) <= n[3] + 0.01 for n in planes) and not any(
            sum((p[k] - q[k]) ** 2 for k in range(3)) < 1e-6 for q in points
        ):
            points.append(p)
    if not 4 <= len(points) <= 252:
        raise ValueError(f"Brush {index}: invalid hull with {len(points)} points")
