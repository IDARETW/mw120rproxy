...
void DrawBsp(void* iterator,const void* context) {
    ...
    uintptr_t world=0,worldName=0;char worldText[64]{};
    if(g_bspSamples.load()<24 &&
       ...
       safemem::ReadString(reinterpret_cast<const char*>(worldName),worldText,sizeof(worldText)) &&
       SelectedWorld(worldText) && g_bspSamples.fetch_add(1)<24) {
        uintptr_t state=0,material=0,technique=0,name=0;char mat[160]{},tech[160]{};unsigned type=~0u;unsigned char layout=255;
        if(safemem::ReadBytes(static_cast<const char*>(context)+8,&state,8) && state) {
            ...
            if(material && safemem::ReadBytes(reinterpret_cast<void*>(material),&name,8))safemem::ReadString(reinterpret_cast<const char*>(name),mat,sizeof(mat));
            if(technique){safemem::ReadBytes(reinterpret_cast<void*>(technique),&name,8);safemem::ReadString(reinterpret_cast<const char*>(name),tech,sizeof(tech));
                safemem::ReadBytes(reinterpret_cast<void*>(technique+8),&type,4);safemem::ReadBytes(reinterpret_cast<void*>(technique+0x9C),&layout,1);}
            LOG_INFO("Render","native BSP dispatch material='%s' technique='%s' type=%u layout=%u acceptedLayout=%d",mat,tech,type,layout,layout>=32&&layout<=38);
        }
    }
    SetLastError(saved);g_bspDraw.load()(iterator,context);
}

void DrawSurfaces(const void* command) {
    const DWORD saved=GetLastError();
    uintptr_t world=0,name=0,view=0,visibility=0;unsigned mask=0;char text[64]{};
    bool capture=false;
    if(safemem::ReadBytes(reinterpret_cast<void*>(g_base+0x10C77870),&world,8) && world &&
       ...
       SelectedWorld(text)) {
        ...
        if(capture){safemem::ReadBytes(command,&view,8);safemem::ReadBytes(reinterpret_cast<void*>(world+0x40B8),&visibility,8);
            if(visibility)safemem::ReadBytes(reinterpret_cast<void*>(visibility),&mask,4);}
    }
    SetLastError(saved);g_draw.load()(command);
    const DWORD resultError=GetLastError();
    if(capture && view) {
        unsigned words=0,draws[8]{};uintptr_t data=0,backend=0,transient=0;unsigned transientMask=0;
        safemem::ReadBytes(reinterpret_cast<void*>(view+0x37B0),&words,4);
        safemem::ReadBytes(reinterpret_cast<void*>(view+0x37B8),&data,8);
        if(data)safemem::ReadBytes(reinterpret_cast<void*>(data),draws,sizeof(draws));
        safemem::ReadBytes(reinterpret_cast<void*>(g_base+0x11108AE8),&backend,8);
        if(backend){safemem::ReadBytes(reinterpret_cast<void*>(backend+0x17317C),&transientMask,4);
            safemem::ReadBytes(reinterpret_cast<void*>(backend+0x173248),&transient,8);}
        LOG_INFO("Render","BSP camera visibility=%08X opaqueWords=%u data=%p draw=%08X,%08X,%08X,%08X transientMask=%08X transient=%p",
            mask,words,reinterpret_cast<void*>(data),draws[0],draws[1],draws[2],draws[3],transientMask,reinterpret_cast<void*>(transient));
    }
    ...
    if(safemem::ReadBytes(static_cast<const char*>(command)+0x7C,&job,4) && job==0 &&
       ...
       SelectedWorld(text)) {
        ...
        if(!g_reported.exchange(true))
            LOG_INFO("Render","selected custom map has no Umbra tome: enabled native conservative visibility fallback view=%u",view);
    }
    SetLastError(saved);
...
hook::Status Install(uintptr_t base) {
    if(g_query.load() && g_draw.load() && g_bspDraw.load())return hook::Status::Installed;
    g_base=base;
    ...
    if(!safemem::ReadBytes(reinterpret_cast<void*>(base+fallback.rva),bytes,fallback.size) ||
       std::memcmp(bytes,fallback.bytes,fallback.size)!=0)return hook::Status::NotReady;
    const auto& query=replay::UmbraQueryStaticVisibilityCmd;
    const auto result=hook::Install(reinterpret_cast<void*>(base+query.rva),&QueryVisibility,query.bytes,query.size,g_query);
    if(result!=hook::Status::Installed)return result;
    const auto& draw=replay::AddBspDrawSurfacesCamera;
    const auto drawn=hook::Install(reinterpret_cast<void*>(base+draw.rva),&DrawSurfaces,draw.bytes,draw.size,g_draw);
    if(drawn!=hook::Status::Installed)return drawn;
    const auto& bsp=replay::DrawBspSurf;
    return hook::Install(reinterpret_cast<void*>(base+bsp.rva),&DrawBsp,bsp.bytes,bsp.size,g_bspDraw);
}
}
