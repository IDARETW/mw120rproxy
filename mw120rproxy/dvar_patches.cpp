...
namespace dvars
{
    namespace
    {
        ...
        const char* ResolveToken(const char* token)
        {
            ...
            while (lo <= hi)
            {
                ...
                if (c == 0) return dvardb::kEntries[mid].name;
        ...
        Entry* FindByName(const char* nm)
        {
            ...
            for (auto& e : g_table)
            {
                if (e.live.load(std::memory_order_acquire) != 1) continue;
        ...
        Entry* Allocate(const char* token, const char* label, bool forced, bool enabled)
        {
            ...
            Entry* slot = nullptr;
            for (auto& e : g_table)
                if (e.live.load(std::memory_order_relaxed) == 0) { slot = &e; break; }
            if (slot)
            {
        ...
        {
            if (!pValue) { _snprintf_s(out, n, _TRUNCATE, "?"); return; }
            if (type == 0)
            {
                uint8_t b;
                if (safemem::ReadBytes(pValue, &b, 1)) _snprintf_s(out, n, _TRUNCATE, "%s", b ? "true" : "false");
                else _snprintf_s(out, n, _TRUNCATE, "?");
                return;
            }
            if (type == 1)
            {
                float f;
                if (safemem::ReadBytes(pValue, &f, 4)) _snprintf_s(out, n, _TRUNCATE, "%g", f);
                else _snprintf_s(out, n, _TRUNCATE, "?");
                return;
            }
            uint32_t raw;
            if (safemem::ReadBytes(pValue, &raw, 4)) _snprintf_s(out, n, _TRUNCATE, "0x%08x", raw);
            else _snprintf_s(out, n, _TRUNCATE, "?");
        }
        ...
        void* __fastcall Detour(const char* name, bool value, unsigned int flags, const char* desc)
        {
            ...
            if (e)
            {
                ...
                if (e->enabled.load(std::memory_order_relaxed))
                    LOG_INFO("Patches/Dvar_RegisterBool", "Patched '%s' -> %s",
                             e->label, newValue ? "true" : "false");
            }
            return result;
        }

        // Tracer: fires for every dvar of every type. Resolves the token to a name.
        void* __fastcall VariantDetour(const char* name, unsigned int checksum, int type,
                                       unsigned int flags, void* pValue, void* pDomain,
                                       const char* desc)
        {
            ...
            if (tn)
                logger::Trace("dvar", "%-46s %-6s = %-22s  tok=%-12s  0x%08X", shown, tn, val, nm, checksum);
            else
                logger::Trace("dvar", "%-46s type%-2d= %-22s  tok=%-12s  0x%08X", shown, type, val, nm, checksum);

            return g_variantOriginal.load(std::memory_order_acquire)(name, checksum, type, flags, pValue, pDomain, desc);
        }
        ...
        bool PrologueMatches(const uint8_t* target, const uint8_t* expected, size_t len)
        {
            ...
            if (!safemem::ReadBytes(target, buf, len)) return false;
            return std::memcmp(buf, expected, len) == 0;
        }

        struct Default { const char* token; const char* name; bool value; };
    ...
    const char* ResolveName(const char* token) { return ResolveToken(token); }
    ...
    void InitDefaults(bool onlineMpRoute, bool luiForceOnline)
    {
        ...
        if (onlineMpRoute)
        {
            ...
            SetForced("MKTLQRMTTS", "online_analytics_streamer_should_be_enabled_based_on_package", false);
            ...
            LOG_INFO("Patches", "online-MP route ON: UI-online dvars forced on, direct-service dvars off");
        }

        LOG_INFO("Patches", "loaded %zu built-in overrides%s; %d dvar names embedded",
                 sizeof(kDefaults) / sizeof(kDefaults[0]), onlineMpRoute ? " (+online-MP route)" : "", dvardb::kCount);
    }
    ...
    hook::Status InstallHook(uintptr_t moduleBase, bool hookBool, bool hookVariant)
    {
        if (hookBool)
        {
            ...
            if (!already) LOG_INFO("Patches", "Dvar_RegisterBool installed at RVA 0x%llX (original published before enable)",
        ...
        if (hookVariant)
        {
            ...
            const auto status = hook::Install(target, &VariantDetour, game::kDvarRegisterVariantPrologue,
                                               game::kDvarRegisterVariantStolen, g_variantOriginal);
            if (status != hook::Status::Installed) return status;
            state::variantHooked.store(true);
            if (!already) LOG_INFO("Patches", "Dvar_RegisterVariant trace installed at RVA 0x%llX",
