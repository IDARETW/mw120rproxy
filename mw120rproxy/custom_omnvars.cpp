...
uintptr_t LoadTables(unsigned char mode, const char* gameType, const char* mapName) {
    ...
    if (mode == 2 && selected && g_customTables.load(std::memory_order_acquire) && before &&
        ...
        LOG_WARN(
            "Omnvars",
            "frontend recovery retains custom tables: clearing native state (%u definitions, %u archived)",
            before, archived);
        SetLastError(saved);
        ClearTables();
    }
    if (custom)
        g_customTables.store(true, std::memory_order_release);
    LOG_INFO(
        "Omnvars",
        "LoadTables mode=%u map='%s' gametype='%s' caller=0x%llX before=%u archived=%u custom=%u",
        mode, map, type, caller >= g_base ? caller - g_base : caller, Count(0x5C48518),
        Count(0x5C4852C), custom);
    SetLastError(saved);
    const auto result = g_load.load(std::memory_order_acquire)(mode, gameType, mapName);
    const DWORD after = GetLastError();
    LOG_INFO("Omnvars", "LoadTables complete map='%s' definitions=%u archived=%u", map,
             Count(0x5C48518), Count(0x5C4852C));
    SetLastError(after);
    return result;
}
}
...
hook::Status Install(uintptr_t base) {
    ...
    if (!safemem::ReadBytes(reinterpret_cast<void*>(base + replay::FrontendOmnvarCall.rva), bytes,
                            replay::FrontendOmnvarCall.size) ||
        std::memcmp(bytes, replay::FrontendOmnvarCall.bytes, replay::FrontendOmnvarCall.size) != 0)
        return hook::Status::NotReady;
    auto status =
        hook::Install(reinterpret_cast<void*>(base + replay::OmnvarClear.rva), &ClearTables,
