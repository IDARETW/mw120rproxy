...
int main(int argc, char** argv) {
    ...
    std::array<unsigned char, 72> input{};
    std::ifstream p(argv[2], std::ios::binary);
    p.read((char*)input.data(), 72);
    if (!p || memcmp(input.data(), "MWRAMB01", 8))
        return 3;
    ...
    D3D12_DESCRIPTOR_RANGE ranges[2]{{D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 0, 0, 0},
                                     {D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 9, 0, 0, 1}};
    ...
    auto constants =
        buffer(D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE);
    auto output = buffer(D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                         D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    auto readback =
        buffer(D3D12_HEAP_TYPE_READBACK, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_FLAG_NONE);
    void* mapped = nullptr;
    Check(constants->Map(0, nullptr, &mapped), "constants map");
    memset(mapped, 0, 256);
    memcpy((char*)mapped + 32, input.data() + 8, 64);
    ...
    list->CopyBufferRegion(readback.Get(), 0, output.Get(), 0, 64);
    ...
    bool same = !memcmp(mapped, input.data() + 8, 64);
    readback->Unmap(0, nullptr);
    if (!same) {
        puts("FAIL native shader probe readback mismatch");
        return 5;
    }
    puts(
        "PASS: actual Replay cs_sh_sample_fallback_probe on D3D12 WARP copies all 64 sky-probe bytes with null light-grid SRVs");
}
