...
class ShaderValidationTests(unittest.TestCase):
    def fixture(self, program):
        header = bytearray(40)
        struct.pack_into("<I", header, 32, len(program))
        return {
            "name": "test",
            "header": header.hex(),
    ...
    def test_rejects_truncated_and_changed_program(self):
        data = b"DXBC" + bytes(32)
        shader = self.fixture(data)
        for bad in (data[:-1], data[:-1] + b"X"):
            with self.assertRaises(ValueError):
                validate_program(shader, bad)

    def test_rejects_non_shader_even_with_matching_hash(self):
        with self.assertRaises(ValueError):
            validate_program(self.fixture(b"nope"), b"nope")
