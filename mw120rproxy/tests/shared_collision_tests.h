...
void SharedCollisionTests() {
    ...
    *reinterpret_cast<uintptr_t*>(image + 0xE5C62F8) = 0;
    ...
    Check(sharedConstructed == 516 && sharedInstances == 15 && sharedFreed == 0,
          "five worlds share 513 child shapes and three compounds");
    ...
    puts(
        "PASS: actual shared collision implementation creates one shape set across five worlds; native call ABI, out-of-order shutdown, final release and reload verified (Havok allocator mocked)");
}
