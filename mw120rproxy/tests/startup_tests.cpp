...
namespace
{
    ...
    {
        auto* image = static_cast<unsigned char*>(VirtualAlloc(nullptr, 0x320000,
            MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE));
        Check(image != nullptr, "allocate GDI compatibility fixture");
        auto* target = image + game::kGdiCloneBootstrapRVA;
        Check(!startup::ApplyGdiCompatibility(reinterpret_cast<uintptr_t>(image)), "unknown startup bytes rejected");
        ...
        Check(startup::ApplyGdiCompatibility(reinterpret_cast<uintptr_t>(image)), "verified GDI constructor repaired");
        Check(target[0] == 0xC3 && std::memcmp(target + 1, game::kGdiCloneBootstrapPrologue + 1,
            sizeof(game::kGdiCloneBootstrapPrologue) - 1) == 0, "only one byte modified");
        Check(startup::ApplyGdiCompatibility(reinterpret_cast<uintptr_t>(image)), "GDI repair is idempotent");
        reinterpret_cast<void(*)()>(target)();
        VirtualFree(image, 0, MEM_RELEASE);
        std::puts("PASS: exact-version GDI repair, mismatch rejection, one-byte scope, idempotence, executable return");
    }
    ...
    bool SplashInstall() { ++calls; return true; }

    void ConcurrentHookTest()
    {
        auto* page = static_cast<unsigned char*>(VirtualAlloc(nullptr, 0x10000,
            MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE));
        ...
        while (calls.load() < 10000) SwitchToThread();
        const unsigned char expected[] = {0x8B, 0x05, 0x0A, 0, 0, 0};
        for (size_t i = 0; i < kCount; ++i)
        {
            Check(hook::Install(reinterpret_cast<void*>(functions[i]), &Detour,
                expected, sizeof(expected), originals[i]) == hook::Status::Installed, "install concurrent hook");
            // Must not recheck the now-patched original bytes and fail on retry.
            Check(hook::Install(reinterpret_cast<void*>(functions[i]), &Detour,
                expected, sizeof(expected), originals[i]) == hook::Status::Installed, "repeat install is idempotent");
        }
        stop.store(true);
        for (auto& worker : workers) worker.join();
        for (active = 0; active < kCount; ++active)
            Check(functions[active]() == 42, "every detour calls relocated original");
        std::printf("PASS: 128 concurrent hooks, four callers, %llu calls, RIP relocation, repeat installation\n", calls.load());
    ...
    void RejectedHookTest()
    {
        ...
        Check(hook::Install(nullptr, &Detour, expected, sizeof(expected), original)
            == hook::Status::Failed, "invalid arguments fail visibly");
        ...
        Check(hook::Install(page, &Detour, expected, sizeof(expected), original)
            == hook::Status::NotReady, "mismatch requests retry");
        Check(!original.load() && page[0] == 0x90, "mismatch publishes no original and writes no bytes");
    ...
    void DvarHookTest()
    {
        ...
        auto* base = static_cast<unsigned char*>(VirtualAlloc(nullptr, 0x1400000,
            MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE));
        ...
        const unsigned char variantBody[] = {0x41,0x5E,0x41,0x5D,0x41,0x5C,0x5F,0x5E,0x5D,0x48,0x8B,0x44,0x24,0x38,0xC3};
        ...
        std::puts("PASS: real 1.20 prologue trampolines, offline switches, stock bools, partial retry, seven-argument ABI");
    }
}
...
int main(int argc, char** argv)
{
    ...
    if (argc > 2)
    {
        ...
        uintptr_t offset=0;
        Check(std::strcmp(replaysymbols::Find(0x161ABE4,offset),"HavokPhysics_AddShapeList")==0 && offset==0x94,"verified fault symbol and offset");
        assetcontext::current.active=true;assetcontext::current.type=29;
    ...
    HANDLE bitmap = LoadImageA(module, MAKEINTRESOURCEA(100), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
    ...
    DeleteObject(bitmap);
    std::puts("PASS: user32 hook, unrelated/missing image rejection, successful splash bitmap, one-shot gate");
    return 0;
}
