...
namespace
{
    ...
    bool __fastcall NativeFixture(void* handle, const char* name)
    {
        ...
        Check(handle == expectedHandle && name == expectedName, "original argument pointers forwarded exactly");
        ++calls;
        if (handle)
        {
...
int main()
{
    ...
    auto* image = static_cast<unsigned char*>(VirtualAlloc(nullptr, size, MEM_RESERVE, PAGE_NOACCESS));
    Check(image != nullptr, "reserve sparse target image");
    auto* page = image + (game::kDbFileOpenRVA & ~uintptr_t{0xFFF});
    Check(VirtualAlloc(page, 0x1000, MEM_COMMIT, PAGE_EXECUTE_READWRITE) != nullptr, "commit function page");
    ...
    const unsigned char tail[] = {0x48,0x8B,0x5C,0x24,0x30,0x48,0x8B,0x74,0x24,0x38,
        0x48,0x83,0xC4,0x20,0x5F,0xFF,0x25,0,0,0,0};
    ...
    Check(fastfilediag::Install(reinterpret_cast<uintptr_t>(image)) == hook::Status::Installed, "checked hook installs");
    Check(fastfilediag::Install(reinterpret_cast<uintptr_t>(image)) == hook::Status::Installed, "repeat install is idempotent");
    auto open = reinterpret_cast<bool(__fastcall*)(void*,const char*)>(target);
    alignas(8) unsigned char handle[24];
    for (const char* name : {"mp_shipment.ff", "ww_mp_hackney_am.ff", "", static_cast<const char*>(nullptr)})
    {
        for (bool opened : {true, false})
        {
            ...
            Check(actual == result && lastError == 0x5678, "native result and exit last error preserved");
            uint32_t index;
            uint64_t cache;
            ...
            Check(handle[4] == 0xCC && handle[7] == 0xCC && handle[16] == 0xCC && handle[23] == 0xCC,
    ...
    result = false;
    SetLastError(0x1234);
    Check(!open(nullptr, expectedName), "unreadable diagnostic input still forwarded to native fixture");
    Check(GetLastError() == 0x5678 && calls == 9, "exactly one original call per request");
    puts("PASS: native DB open prologue, mismatch/retry, exact pointer forwarding, success/failure handles, last-error state and unreadable diagnostic inputs");
