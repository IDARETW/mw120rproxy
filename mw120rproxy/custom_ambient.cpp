...
bool Probe(const std::string& id, std::array<unsigned char, 64>& probe) {
    for (const auto& p : custommaps::List())
        if (p.valid && p.id == id) {
...
void UpdateProbe(void* state, const void* data, const void* view) {
    uintptr_t world = 0, name = 0;
    char text[96]{};
    const auto id = custommaps::Active();
    const bool custom =
        !id.empty() &&
    ...
    if (!custom) {
        ...
        if (g_custom) {
            *reinterpret_cast<unsigned char*>(g_base + 0xFB942B8) = 1;
            g_custom = false;
            g_world.clear();
            g_resource = 0;
        }
        g_update.load()(state, data, view);
        return;
    }
    unsigned index = ~0u, count = 0;
    uintptr_t shader = 0, resource = 0;
    if (!safemem::ReadBytes(static_cast<const char*>(data) + 0x113164, &index, 4) || index > 1 ||
        !safemem::ReadBytes(reinterpret_cast<void*>(g_base + 0xFB94360 + index * 0xF8), &count,
                            4) ||
        count) {
        g_update.load()(state, data, view);
        return;
    ...
    if (!safemem::ReadBytes(reinterpret_cast<void*>(g_base + 0x10C78D40), &shader, 8) || !shader ||
        !safemem::ReadBytes(reinterpret_cast<void*>(g_base + 0xFB94258), &resource, 8) || !resource)
        return;
    if (g_custom && g_world == id && g_resource == resource)
        return;
    ...
    uintptr_t device = 0;
    if (!compute || !safemem::ReadBytes(static_cast<char*>(compute) + 0xC50, &device, 8))
        return;
    reinterpret_cast<void (*)(uintptr_t)>(g_base + replay::LockGfxImmediate.rva)(device);
    reinterpret_cast<void (*)(void*, void*)>(g_base + replay::SetComputeShader.rva)(
        compute, reinterpret_cast<void*>(shader));
    ...
    alignas(16) std::array<unsigned char, 96> constants{};
    memcpy(constants.data() + 32, probe.data(), 64);
    reinterpret_cast<void (*)(void*, unsigned, const void*, unsigned, void*)>(
        g_base + replay::UploadComputeConstants.rva)(compute, 0, constants.data(), 96, nullptr);
    const void* rw = reinterpret_cast<void*>(g_base + 0xFB94298);
    reinterpret_cast<void (*)(void*, unsigned, unsigned, const void* const*, const unsigned*)>(
        g_base + replay::SetComputeRWViews.rva)(compute, 0, 1, &rw, nullptr);
    reinterpret_cast<void (*)(void*, unsigned, unsigned, unsigned)>(
        g_base + replay::ComputeDispatch.rva)(compute, 1, 1, 1);
    ...
    LOG_INFO(
        "Ambient",
        "uploaded sky-derived fallback SH probe map=%s (native arms, weapons and dynamic models)",
        id.c_str());
}
}
...
hook::Status Install(uintptr_t base) {
    ...
    return hook::Install(reinterpret_cast<void*>(base + replay::UpdateFallbackProbe.rva),
                         &UpdateProbe, replay::UpdateFallbackProbe.bytes,
                         replay::UpdateFallbackProbe.size, g_update);
}
}
