...
inline Settings Load(HMODULE self) {
    ...
    if (_snwprintf_s(ini, _countof(ini), _TRUNCATE, L"%smw120rproxy.ini", path) <= 0)
        return s;
    ...
    wchar_t trig[64]{};
    GetPrivateProfileStringW(L"mw120rproxy", L"hook_trigger", L"loadimage", trig, _countof(trig),
                             ini);
    char trigA[64]{};
