...
namespace
{
    ...
    DWORD WINAPI EngineConsoleWorker(void*) {
        ...
        for(;;) {
            ...
            for(unsigned batch=0;batch<256;++batch) {
                ...
                if(!g_engineCount){ReleaseSRWLockExclusive(&g_engineLock);break;}
                memcpy(line,g_engineLines[g_engineHead],sizeof(line));
                g_engineHead=(g_engineHead+1)%256;--g_engineCount;
                ReleaseSRWLockExclusive(&g_engineLock);
                fputs(line,stdout);
            }
            const auto now=GetTickCount64();
            if(now-lastReport>=1000) {
                lastReport=now;const auto count=g_consoleSuppressed.exchange(0);
                if(count)fprintf(stdout,"[Engine] %u repeated/burst messages omitted from this window; see mw120rproxy.engine.log\n",count);
            }
            fflush(stdout);
        }
    }
    ...
    void Timestamp(char* out, size_t n)
    {
        SYSTEMTIME st;
        GetLocalTime(&st);
        _snprintf_s(out, n, _TRUNCATE, "%02d:%02d:%02d.%03d",
...
namespace log120r
{
    void Init(HMODULE self, bool openConsole)
    {
        ...
        if (len && len < MAX_PATH)
        {
            ...
            if (FILE* f = nullptr; _wfopen_s(&f, g_logPath.c_str(), L"w") == 0 && f) fclose(f);
            if (FILE* f = nullptr; _wfopen_s(&f, g_tracePath.c_str(), L"w") == 0 && f) fclose(f);
        }
        ...
        if(g_console) {
            ...
            DWORD mode=0;const HANDLE input=GetStdHandle(STD_INPUT_HANDLE);
            ...
            if(g_engineEvent) {
                ...
                if(GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|GET_MODULE_HANDLE_EX_FLAG_PIN,
                    ...
                    if(thread){CloseHandle(thread);g_engineConsoleReady.store(true,std::memory_order_release);}
                }
            }
        }
    }
    ...
    void EngineConsole(unsigned channel,int flags,const char* text) {
        if(!g_engineConsoleReady.load(std::memory_order_acquire)||!text||!*text)return;
        const DWORD saved=GetLastError();
        const auto now=GetTickCount64();
        const auto hash=engineconsole::Hash(channel,flags,text);
        if(!TryAcquireSRWLockExclusive(&g_engineLock)){++g_consoleSuppressed;SetLastError(saved);return;}
        if(g_engineCount==256 || !g_engineGate.Accept(hash,flags==3,now)) {
            ++g_consoleSuppressed;ReleaseSRWLockExclusive(&g_engineLock);SetLastError(saved);return;
        }
        char clean[1750]{};size_t n=0;
        for(size_t i=0;text[i]&&n<sizeof(clean)-1;++i) {
            if(text[i]=='^'&&text[i+1]>='0'&&text[i+1]<='9'){++i;continue;}
            const auto c=static_cast<unsigned char>(text[i]);
            if(c>=32||c=='\n'||c=='\t')clean[n++]=static_cast<char>(c);
        }
        ...
        _snprintf_s(line,2048,_TRUNCATE,"[%s] [Engine ch=0x%X %s] %s%s",ts,channel,
            flags==3?"ERROR":flags==2?"WARN":"DEBUG",clean,n&&clean[n-1]=='\n'?"":"\n");
        ++g_engineCount;ReleaseSRWLockExclusive(&g_engineLock);
        SetEvent(g_engineEvent);SetLastError(saved);
    }
