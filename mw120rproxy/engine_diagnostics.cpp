...
namespace
{
    ...
    void Write(bool error, const char* format, ...)
    {
        ...
        {
            const char limit[] = "Routine engine output capped at 64 MiB; fatal diagnostics remain enabled.\r\n";
            WriteFile(g_file,limit,sizeof(limit)-1,&written,nullptr);
            g_limitReported = true;
        }
        if (error) FlushFileBuffers(g_file);
        ReleaseSRWLockExclusive(&g_lock);
        if(error)log120r::EngineConsole(0,3,line);
    }
    void Stack()
    {
        void* frames[32]{};
        ...
        for (USHORT i=0;i<count;++i)
        {
            ...
            if (pc>=g_base && pc-g_base<0x1324B000)
            {
                ...
                if (name) Write(true,"  frame[%u] game RVA 0x%llX %s+0x%llX\r\n",i,pc-g_base,name,offset);
                else Write(true,"  frame[%u] game RVA 0x%llX\r\n",i,pc-g_base);
            }
            else
            {
                ...
                char path[MAX_PATH]{};
                if(info.Type==MEM_IMAGE) GetModuleFileNameA(static_cast<HMODULE>(info.AllocationBase),path,MAX_PATH);
                Write(true,"  frame[%u] pc=%p module=%s offset=0x%llX\r\n",i,frames[i],path,pc-reinterpret_cast<uintptr_t>(info.AllocationBase));
            }
        }
    }
    template<class T> T Read(uintptr_t address)
    {
        T value{}; safemem::ReadBytes(reinterpret_cast<void*>(address),&value,sizeof(value)); return value;
    }
    uintptr_t LinkAssetEntry(int type,uintptr_t* header)
    {
        ...
        context={};context.active=true;context.type=type;context.phase="DB_LinkXAssetEntry";
        ...
        if(type==23 || type==29)
        {
            ...
            Write(false,"ASSET BEGIN type=%d name='%s' asset=%p HavokData=%p HavokBytes=%u\r\n",type,context.name,
                reinterpret_cast<void*>(context.asset),reinterpret_cast<void*>(context.shapeData),context.shapeBytes);
        }
        else if(type==61)
        {
            Write(false,"ASSET BEGIN NetConstStrings name='%s' stringType=%u sourceType=%u flags=0x%X entries=%u\r\n",
    ...
    uintptr_t PrintMessage(unsigned channel,const char* text,int flags)
    {
        ...
        if(error && g_printErrors.fetch_add(1)<16) Stack();
        SetLastError(saved);
    ...
    uintptr_t Disc(uintptr_t reader,unsigned major,unsigned minor)
    {
        ...
        if(g_discErrors.fetch_add(1)<16)
        {
            ...
            Write(true,"DB_DiscError [%u.%u] file='%s' reader=%p descriptor=%p\r\n",major,minor,filename,reinterpret_cast<void*>(reader),reinterpret_cast<void*>(descriptor));
            const auto state=Read<uintptr_t>(reader);
            Write(true,"  sizeRead=%llu sizeInflated=%llu sizePassedToInflator=%llu fileSize=%llu signed=%u\r\n",
                  Read<uint64_t>(reader+0xB8),Read<uint64_t>(reader+0xC0),Read<uint64_t>(reader+0xC8),Read<uint64_t>(reader+0xD0),Read<uint8_t>(descriptor+0x50));
            ...
            if(index<count && count<=20)
            {
                ...
                Write(true,"  readBuffer=%p requested=%llu offset=%llu state=%u actual=%llu result=%u\r\n",reinterpret_cast<void*>(buffer),
                      Read<uint64_t>(buffer+8),Read<uint64_t>(buffer+16),Read<unsigned>(buffer+24),Read<uint64_t>(buffer+32),Read<unsigned>(buffer+40));
            }
            Write(true,"  decoder avail_in=%llu total_in=%llu avail_out=%llu total_out=%llu compressor=%u authKind=%u\r\n",
                  Read<uint64_t>(state+0x8E8F0),Read<uint64_t>(state+0x8E8F8),Read<uint64_t>(state+0x8E908),
                  Read<uint64_t>(state+0x8E910),Read<unsigned>(state+0x8E918),Read<unsigned>(state+0x8E980));
            Stack();
        }
        SetLastError(saved);
    ...
    void Com(int code,const char* format,va_list args)
    {
        ...
        {
            char text[2048]{};safemem::ReadString(format,text,sizeof(text));
            Write(true,"Com_Error_Internal code=%d format='%s' (native formatting follows)\r\n",code,text);
            Stack();
        }
        SetLastError(saved);
        g_comError.load(std::memory_order_acquire)(code,format,args); // va_list is never consumed here
    }
    void Fatal(const char* text)
    {
        ...
        {
            char message[4096]{};safemem::ReadString(text,message,sizeof(message));
...
namespace enginediag
{
    bool Initialize(HMODULE self)
    {
        ...
        g_file=CreateFileW(path,FILE_APPEND_DATA,FILE_SHARE_READ|FILE_SHARE_WRITE,nullptr,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,nullptr);
        return g_file!=INVALID_HANDLE_VALUE;
    }
    hook::Status Install(uintptr_t base)
    {
        g_base=base;
        auto status=hook::Install(reinterpret_cast<void*>(base+replay::PrintMessage.rva),&PrintMessage,replay::PrintMessage.bytes,replay::PrintMessage.size,g_print);
        if(status!=hook::Status::Installed)return status;
        status=hook::Install(reinterpret_cast<void*>(base+replay::DiscError.rva),&Disc,replay::DiscError.bytes,replay::DiscError.size,g_disc);
        if(status!=hook::Status::Installed)return status;
        status=hook::Install(reinterpret_cast<void*>(base+replay::ComErrorInternal.rva),&Com,replay::ComErrorInternal.bytes,replay::ComErrorInternal.size,g_comError);
        if(status!=hook::Status::Installed)return status;
        status=hook::Install(reinterpret_cast<void*>(base+replay::FatalText.rva),&Fatal,replay::FatalText.bytes,replay::FatalText.size,g_fatalText);
        if(status!=hook::Status::Installed)return status;
        status=hook::Install(reinterpret_cast<void*>(base+replay::LinkAssetEntry.rva),&LinkAssetEntry,replay::LinkAssetEntry.bytes,replay::LinkAssetEntry.size,g_linkAsset);
        if(status==hook::Status::Installed)Write(false,"Replay engine output, named error call chains and per-thread asset context installed.\r\n");
        return status;
    }
}
