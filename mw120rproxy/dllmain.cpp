// MW2019 1.20 Replay: minimal DllMain, checked startup hooks, owner-run testing.
#include <windows.h>
#include <mutex>
#include "proxy.h"
#include "log.h"
...
namespace
{
    ...
    bool CheckTarget(uintptr_t base, uint32_t& size)
    {
        ...
        if (!name || _wcsicmp(name + 1, L"game_dx12_ship_replay.exe") != 0) return false;
    ...
    bool DoInstallHooks()
    {
        ...
        if (g_finished) return true;
        if (!g_splashSeen.load(std::memory_order_acquire)) return false;
        ...
        if (status == hook::Status::NotReady) return false;
        g_finished = true;
        state::hookState.store((int)(status == hook::Status::Installed
            ? state::HookState::Installed : state::HookState::Failed));
        if (status == hook::Status::Installed) LOG_INFO("Core", "enabled startup hooks installed; waiting for game registration");
        else LOG_ERR("Core", "hook installation failed; inspect preceding error (not reported as installed)");
        return true;
    }
    ...
    bool OnSplashLoaded()
    {
        ...
        if (g_ready.load(std::memory_order_acquire)) DoInstallHooks();
        return true;
    }
    ...
    DWORD WINAPI StartupMonitor(LPVOID)
    {
        ...
        while (!g_splashSeen.load(std::memory_order_acquire))
        {
            ...
            {
                LOG_ERR("Trigger", "game splash not observed; no game hooks installed (no polling fallback)");
                return 0;
            }
            Sleep(10);
        }
        ...
        {
            Sleep(5000);
            LOG_INFO("Startup", "post-splash=%ds hook_state=%d bool_calls=%llu variant_calls=%llu (menu state requires owner confirmation)",
                (i + 1) * 5, state::hookState.load(), state::boolDvarsSeen.load(), state::dvarsSeen.load());
            if (g_config.offlineAuth) offlineauth::LogSnapshot();
        }
        return 0;
    }
    ...
    DWORD WINAPI InitThread(LPVOID)
    {
        ...
        try
        {
            ...
            LOG_INFO("Core", "mw120rproxy custom-map-authoring-v29 2026-09-07; mapInfo CSV backgrounds for hover and loading screens; HDR sky ambient; authored footsteps; target 1.20.4.7623265-replay");
            LOG_INFO("Diagnostics", "exception call chains: mw120rproxy.exceptions.log and per-run mw120rproxy.crash.*.log; native errors: mw120rproxy.engine.log");
            LOG_INFO("Startup", "GDI clone compatibility repair at RVA 0x3061A0: %s", g_gdiCompatibility ? "applied" : "FAILED (target bytes/protection mismatch)");
            LOG_INFO("Core", "config: hook_bool=%d hook_variant=%d offline_auth=%d map_monitor=%d map_redirect=%d trigger=%s",
                (int)g_config.hookBool, (int)g_config.hookVariant, (int)g_config.offlineAuth, (int)g_config.customMapFileMonitor,
                (int)g_config.customMapFileRedirect, g_config.hookTrigger.c_str());
            proxy::Preload();
            LOG_INFO("Core", "fastfile_diagnostics=%d", int(g_config.fastfileDiagnostics));
            LOG_INFO("Core", "developer_ui=%d custom_map_loader=%d", int(g_config.developerUI), int(g_config.customMapLoader));
            ...
            if (g_config.offlineAuth && (!g_config.hookBool || !offlineauth::Prepare(g_self)))
            {
                ...
                LOG_ERR("Auth", "local identity preparation failed or hook_bool=0; check mw120rproxy.identity.ini (must contain a nonzero decimal xuid)");
                return 0;
            }
            if (g_config.hookBool || g_config.hookVariant || g_config.offlineAuth || g_config.fastfileDiagnostics ||
                g_config.customMapFileMonitor || g_config.developerUI || g_config.customMapLoader || g_config.engineDiagnostics)
            {
                ...
                LOG_INFO("Trigger", "user32 LoadImageA armed during attach; waiting for bitmap 100 at game RVA 0x1BD3479");
                if (g_config.hookTrigger != "loadimage")
                    LOG_WARN("Trigger", "hook_trigger=%s ignored; splash gate is mandatory", g_config.hookTrigger.c_str());
                g_ready.store(true, std::memory_order_release);
                HANDLE monitor = CreateThread(nullptr, 0, StartupMonitor, nullptr, 0, nullptr);
                if (monitor) CloseHandle(monitor);
                else LOG_ERR("Core", "startup monitor thread failed (%lu)", GetLastError());
            }
