...
namespace startup
{
    bool ApplyGdiCompatibility(uintptr_t imageBase)
    {
        ...
        if (!safemem::ReadBytes(target, actual, sizeof(actual))) return false;
        ...
        if (actual[0] == 0xC3 && std::memcmp(actual + 1, game::kGdiCloneBootstrapPrologue + 1, sizeof(actual) - 1) == 0)
            return true;
        if (std::memcmp(actual, game::kGdiCloneBootstrapPrologue, sizeof(actual)) != 0) return false;
        DWORD old = 0;
