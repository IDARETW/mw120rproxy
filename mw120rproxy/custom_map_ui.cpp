...
void Load() {
    ...
    int count = 0;
    if (!safemem::ReadBytes(reinterpret_cast<void*>(g_base + CountRva), &count, 4) || count <= 0 ||
        count > Capacity)
        return;
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (!safemem::ReadBytes(rows + i * Stride, row.data(), row.size()))
            return;
        if (memcmp(row.data() + 0x20, "mp_shipment\0", 12) == 0) {
            reference = row;
            found = true;
            break;
        }
    ...
    if (!found) {
        LOG_WARN("Maps", "native map list has no Shipment template; custom menu entries deferred");
        return;
    }
    unsigned added = 0;
    for (const auto& package : custommaps::List()) {
        if (!package.valid || package.id.size() >= 16 || count >= Capacity)
            continue;
        bool duplicate = false;
        for (int i = 0; i < count; ++i)
            if (memcmp(rows + i * Stride + 0x20, package.id.c_str(), package.id.size() + 1) == 0) {
                duplicate = true;
                break;
            }
        if (duplicate)
        ...
        const int aliens = 0;
        memcpy(row.data() + 0x4D0, &aliens, 4);
        memcpy(rows + count * Stride, row.data(), Stride);
        ++count;
        ++added;
        LOG_INFO("Maps", "added normal Local Play map entry id=%s title=%s", package.id.c_str(),
                 package.title.c_str());
    }
...
void SyncSelection(const char* map) {
    ...
    if (custommaps::IsKnownMap(map)) {
        if (custommaps::Select(map))
            LOG_INFO("Maps", "normal UI selected custom package %s", map);
