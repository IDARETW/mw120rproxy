...
namespace
{
    ...
    {
        if (id.size() < 4 || id.size() > 63 || id.rfind("mp_", 0) != 0) return false;
    ...
    bool SkipJsonValue(const std::string& text, size_t& pos, unsigned int depth)
    {
        ...
        if (text[pos] == '[')
        {
            ...
            for (;;)
            {
                if (!SkipJsonValue(text, pos, depth + 1)) return false;
                SkipSpace(text, pos);
                if (pos >= text.size()) return false;
                if (text[pos] == ']')
                {
        ...
        if (text[pos] == '{')
        {
            ...
            {
                std::string key;
                if (!ReadString(text, pos, key)) return false;
                SkipSpace(text, pos);
                if (pos >= text.size() || text[pos++] != ':') return false;
                if (!SkipJsonValue(text, pos, depth + 1)) return false;
                SkipSpace(text, pos);
    ...
    {
        size_t pos = 0;
        if (!SkipJsonValue(text, pos, 0)) return false;
        SkipSpace(text, pos);
    ...
    bool KeyPosition(const std::string& text, std::string_view key, size_t& valuePos)
    {
        ...
        {
            SkipSpace(text, pos);
            if (pos >= text.size() || text[pos] == '}') return found;
            std::string name;
            if (!ReadString(text, pos, name)) return false;
            SkipSpace(text, pos);
            if (pos >= text.size() || text[pos++] != ':') return false;
            if (name == key)
            {
    ...
    {
        size_t pos = 0;
        if (!KeyPosition(text, key, pos)) return true;
    ...
    {
        size_t pos = 0;
        if (!KeyPosition(text, "schema", pos)) return false;
        SkipSpace(text, pos);
        if (pos >= text.size() || text[pos] != '1') return false;
        ++pos;
        SkipSpace(text, pos);
    ...
    {
        size_t pos = 0;
        if (!KeyPosition(text, "gametypes", pos)) return false;
        SkipSpace(text, pos);
        if (pos >= text.size() || text[pos++] != '[') return false;
        ...
        for (;;)
        {
            ...
            if (text[pos] == ']') return false;
            std::string value;
            if (!ReadString(text, pos, value)) return false;
            if (value == "tdm") return true;
            SkipSpace(text, pos);
            if (pos >= text.size() || text[pos] != ',') return false;
            ++pos;
        }
    ...
    bool ReadText(const std::filesystem::path& path, std::string& out)
    {
        ...
        if (size < 2 || size > 64 * 1024) return false;
    ...
    bool HasReplayFastfileHeader(const std::filesystem::path& path)
    {
        ...
        if (error || fileSize < 0x8C || fileSize > UINT32_MAX) return false;
        std::ifstream input(path, std::ios::binary);
        std::array<uint8_t, 0xA0> header{};
        if (!input.read(reinterpret_cast<char*>(header.data()), std::min<uintmax_t>(header.size(),fileSize))) return false;
        const bool stored=std::memcmp(header.data(),"IWffc100",8)==0;
        if (!stored && std::memcmp(header.data(), "IWffa100", 8) != 0) return false;
        ...
        for (size_t index = 0; index < 11; ++index)
        {
            ...
            std::memcpy(&blockSize, header.data() + 0x30 + index * sizeof(blockSize), sizeof(blockSize));
            streamBytes += blockSize;
        }
        const bool body = stored ? compression==0 &&
            std::memcmp(header.data()+0x88,"\x01IWC",4)==0 && residentPartSize==xfileSize+4 :
            fileSize>=0xA0 && compression==1 && std::memcmp(header.data()+0x88,"IWffs100",8)==0;
    ...
    bool HasStockShaderHeader(const std::filesystem::path& path)
    {
        if (GetFileAttributesW(path.c_str()) == INVALID_FILE_ATTRIBUTES ||
            (GetFileAttributesW(path.c_str()) & FILE_ATTRIBUTE_REPARSE_POINT) != 0) return false;
        std::ifstream file(path, std::ios::binary);
        std::array<uint8_t, 20> header{};
        if (!file.read(reinterpret_cast<char*>(header.data()), header.size())) return false;
        const uint8_t expected[]{'I','W','f','f','a','1','0','0',11,0,0,0,0xF7,0x0F,0,0,1,0,0,0};
        return std::memcmp(header.data(), expected, sizeof(expected)) == 0;
    }

    std::filesystem::path ZonePath(const custommaps::Package& package, const std::string& zone,
    ...
    bool ZoneFromRequest(const char* request, std::string& zoneOut)
    {
        ...
        if (name.size() > 3 && name.ends_with(".ff")) name.resize(name.size() - 3);
        if (name.empty() || (name != request && path.extension() != ".ff")) return false;
    ...
    void ValidatePackage(custommaps::Package& package, const std::filesystem::path& directory)
    {
        ...
        if(!OptionalString(text,"collision",collision) || (!collision.empty() && collision!="boxes-v1" && collision!="convex-v2")) {
            package.error="unsupported collision format";return;
        }
        ...
        if(!collision.empty() || std::filesystem::exists(collisionPath)) {
            ...
            if((GetFileAttributesW(collisionPath.c_str())&FILE_ATTRIBUTE_REPARSE_POINT) || !collisionfile::Load(collisionPath,brushes)) {
                package.error="collision.bin is missing or invalid";return;
            }
        }
        std::string glass;
        if(!OptionalString(text,"glass",glass) || (!glass.empty() && glass!="panes-v1" && glass!="panes-v2")) {package.error="unsupported glass format";return;}
        if(!glass.empty() || std::filesystem::exists(directory/"glass.bin")) {
            std::vector<glassfile::Pane> panes;unsigned surfaces=0;
            if(!glassfile::Load(directory/"glass.bin",panes,surfaces)){package.error="glass.bin is missing or invalid";return;}
        }
        std::string ladders;
        if(!OptionalString(text,"ladders",ladders) || (!ladders.empty() && ladders!="faces-v1" && ladders!="faces-v2")) {
            package.error="unsupported ladder format";return;
        }
        ...
        if(!ladders.empty() || std::filesystem::exists(ladderPath)) {
            ...
            if((GetFileAttributesW(ladderPath.c_str())&FILE_ATTRIBUTE_REPARSE_POINT) || !ladderfile::Load(ladderPath,faces)) {
                package.error="ladders.bin is missing or invalid";return;
            }
        }
        package.valid = true;
    }
    ...
    void ScanLocked()
    {
        ...
        std::sort(packages.begin(), packages.end(), [](const custommaps::Package& left,
                                                        const custommaps::Package& right)
        {
            return left.title < right.title;
        });

        const bool selectionStillExists = std::any_of(packages.begin(), packages.end(),
            [](const custommaps::Package& package)
            {
                return package.valid && package.id == g_selected;
            });
        if (!selectionStillExists) g_selected.clear();
...
namespace custommaps
{
    ...
    bool Select(const char* id)
    {
        if (!id) return false;
        std::lock_guard<std::mutex> lock(g_lock);
        const auto found = std::find_if(g_packages.begin(), g_packages.end(), [id](const Package& package)
        {
            return package.valid && package.id == id;
        });
        if (found == g_packages.end()) return false;
        g_selected = found->id;
    ...
    {
        if (!id) return "map id is missing";
        std::lock_guard<std::mutex> lock(g_lock);
        const auto found = std::find_if(g_packages.begin(), g_packages.end(), [id](const Package& package)
        {
            return package.id == id;
        });
        if (found == g_packages.end()) return "map package was not found";
    ...
    bool ActiveZonePath(const char* zoneName, std::wstring& pathOut)
    {
        ...
        std::lock_guard<std::mutex> lock(g_lock);
        const auto package = std::find_if(g_packages.begin(), g_packages.end(), [](const Package& item)
        {
            return item.valid && item.id == g_selected;
        });
        if (package == g_packages.end()) return false;
        for (const std::string& zone : ZoneNames(package->id))
        {
            if (zone == zoneName)
            {
    ...
    {
        if (!qpathOut || qpathOutSize == 0) return false;
        qpathOut[0] = 0;

        std::string zone;
        if (!ZoneFromRequest(request, zone)) return false;
        ...
        if (std::find(names.begin(), names.end(), zone) == names.end()) return false;
    ...
    bool ResolveDiskRead(const char* request, std::string& pathOut)
    {
        pathOut.clear();
        if (!request || !*request) return false;
        const auto source = std::filesystem::path(std::u8string(reinterpret_cast<const char8_t*>(request)));
        ...
        for (const auto& zone : ZoneNames(package->id))
        {
            ...
            if (extension != ".ff" && !(shader && extension == ".xpak")) continue;
            const auto filename = zone + extension;
            for (const auto& folder : {std::filesystem::path{}, std::filesystem::path{"zone"},
                 std::filesystem::path{"zone/english"}, std::filesystem::path{"zone/worldwide"}})
            {
                ...
                if (_wcsicmp(full.c_str(), expected.c_str()) != 0) continue;
                ...
                if (GetFileAttributesW(target.c_str()) == INVALID_FILE_ATTRIBUTES ||
                    ...
                    (GetFileAttributesW(package->directory.c_str()) & FILE_ATTRIBUTE_REPARSE_POINT) != 0) return false;
