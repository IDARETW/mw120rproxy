...
namespace hook
{
    ...
    {
        std::mutex& Mutex();
        Status Prepare(void* target, void* detour, const uint8_t* expected,
                       size_t expectedSize, void** trampoline);
    ...
    Status Install(void* target, Fn detour, const uint8_t* expected,
                   size_t expectedSize, std::atomic<Fn>& original)
    {
        std::lock_guard<std::mutex> lock(detail::Mutex());
        if (original.load(std::memory_order_acquire)) return Status::Installed;
        void* trampoline = nullptr;
        const Status status = detail::Prepare(target, reinterpret_cast<void*>(detour),
                                              expected, expectedSize, &trampoline);
        if (status != Status::Installed) return status;
        original.store(reinterpret_cast<Fn>(trampoline), std::memory_order_release);
        if (detail::Enable(target)) return Status::Installed;
