...
namespace trigger
{
    namespace
    {
        ...
        // Verified on this host (2026-09-07), plus the older supported Windows prologue.
        constexpr uint8_t kCurrent[] = {0x48,0x89,0x5C,0x24,0x08,0x48,0x89,0x6C,0x24,0x18,
            0x56,0x57,0x41,0x54,0x41,0x56,0x41,0x57,0x48,0x83,0xEC,0x30};
        constexpr uint8_t kLegacy[] = {0x48,0x89,0x5C,0x24,0x08,0x48,0x89,0x74,0x24,0x18,
        ...
        HANDLE WINAPI Detour(HINSTANCE instance, LPCSTR name, UINT type, int cx, int cy, UINT flags)
        {
            ...
            if (splash && result && !g_delivered.exchange(true))
            {
                ...
                LOG_INFO("Trigger", "game splash bitmap 100 loaded; caller RVA=0x%llX; releasing game-hook gate",
                    (unsigned long long)(caller - g_imageBase));
                if (g_install) g_install();
            }
            SetLastError(error);
            return result;
        }
    ...
    bool InstallLoadImageTrigger(InstallFn install, uintptr_t imageBase, uintptr_t splashReturnRva)
    {
        ...
        if (!target || !safemem::ReadBytes(target, actual, sizeof(actual))) return false;
        const uint8_t* expected = nullptr;
        size_t size = 0;
        if (std::memcmp(actual, kCurrent, sizeof(kCurrent)) == 0) { expected = kCurrent; size = sizeof(kCurrent); }
        else if (std::memcmp(actual, kLegacy, sizeof(kLegacy)) == 0) { expected = kLegacy; size = sizeof(kLegacy); }
