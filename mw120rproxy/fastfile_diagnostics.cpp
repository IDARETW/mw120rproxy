...
bool __fastcall ObserveOpen(DBFileHandle* handle, const char* filename) {
    ...
    if ((opened && count < 256) || (!opened && count < 32)) {
        ...
        logger::Trace(
            "Fastfile",
            "DB_File_OpenDBFile name='%s' opened=%d handle_readable=%d stream=0x%08X dcache=0x%llX caller_rva=0x%llX",
            name, int(opened), int(readable), result.fileStreamIndex,
            (unsigned long long)result.dcacheId, (unsigned long long)callerRva);
        if (!opened) {
            LOG_WARN(
                "Fastfile",
                "native open failed for '%s'; caller RVA 0x%llX, stream=0x%08X dcache=0x%llX (stock result preserved; optional probes can fail)",
...
hook::Status Install(uintptr_t moduleBase) {
    ...
    const auto status =
        hook::Install(reinterpret_cast<void*>(moduleBase + game::kDbFileOpenRVA), &ObserveOpen,
                      game::kDbFileOpenPrologue, sizeof(game::kDbFileOpenPrologue), g_original);
    if (status == hook::Status::Installed && !already)
        LOG_INFO(
            "Fastfile",
            "native DB_File_OpenDBFile observer installed at RVA 0xD8CA30; paths and results unchanged");
    return status;
}
}
