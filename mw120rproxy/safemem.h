...
// safe when the game has a vectored exception handler
...
namespace safemem
{
    ...
    inline size_t ReadString(const char* src, char* dst, size_t dstSize)
    {
        ...
        if (!src) return 0;
        size_t avail = 0;
        if (!RegionReadable(src, avail)) return 0;
        size_t maxN = dstSize - 1;
        if (maxN > avail) maxN = avail;
        size_t i = 0;
        for (; i < maxN && src[i]; ++i) dst[i] = src[i]; // inside a committed page -> safe
        dst[i] = 0;
        return i;
    }
    ...
    {
        size_t avail = 0;
        if (!RegionReadable(src, avail) || avail < len) return false;
    ...
    {
        if (!dst || !RegionWritable(dst)) return false;
    ...
    inline bool WriteBytes(void* dst, const void* src, size_t len)
    {
        ...
        MEMORY_BASIC_INFORMATION mbi{};
        if (VirtualQuery(dst, &mbi, sizeof(mbi)) != sizeof(mbi)) return false;
