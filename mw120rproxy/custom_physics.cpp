...
bool EmptySelected(char** data, unsigned* size, const char* name, int type, uintptr_t caller) {
    ...
    if (type == 29 && caller == g_base + 0x1490C4F) {
        field = 0x150;
        countField = 0x158;
    } else if (type == 23 && caller == g_base + 0x1490AB0) {
        field = 0xC0;
        countField = 0x0C;
    } else
        return false;
    const auto asset = reinterpret_cast<uintptr_t>(data) - field;
    if (reinterpret_cast<uintptr_t>(size) != asset + field - 8)
        return false;
    const char* assetName = nullptr;
    unsigned count = ~0u;
    if (!safemem::ReadBytes(reinterpret_cast<void*>(asset), &assetName, sizeof(assetName)) ||
        assetName != name ||
        !safemem::ReadBytes(reinterpret_cast<void*>(asset + countField), &count, sizeof(count)) ||
        count)
        return false;
    char text[160]{};
    const auto length = safemem::ReadString(name, text, sizeof(text));
    if (!length || length == sizeof(text) - 1)
        return false;
    try {
...
void* AddShapeList(char** data, unsigned* size, const char* name, int type) {
    ...
    unsigned bytes = 0;
    const bool readable = safemem::ReadBytes(data, &payload, sizeof(payload)) &&
                          safemem::ReadBytes(size, &bytes, sizeof(bytes));
...
hook::Status Install(uintptr_t base) {
    ...
    g_base = base;
    if (!Matches(replay::MapShapeCall) || !Matches(replay::ClipShapeCall) ||
        !Matches(replay::SetMainShapeList))
