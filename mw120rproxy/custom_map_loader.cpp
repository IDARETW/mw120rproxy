...
namespace
{
    ...
    uintptr_t __fastcall ResetSelectedReader(uintptr_t reader)
    {
        ...
        if (safemem::ReadBytes(reinterpret_cast<void*>(reader),&state,sizeof(state)) && state &&
            safemem::ReadBytes(reinterpret_cast<void*>(reader+0xB0),&descriptor,sizeof(descriptor)) && descriptor &&
            safemem::ReadBytes(reinterpret_cast<void*>(descriptor+0x50),&signedFile,1) && signedFile==1 &&
        ...
        {
            const auto count=safemem::ReadString(reinterpret_cast<const char*>(descriptor),name,sizeof(name));
            try {
                ...
                if(count && count<sizeof(name)-1 && zone.ends_with(".ff")) {
                    ...
                    if(custommaps::ActiveShaderSource(zone.c_str(),source)) {
                        ...
                        safemem::ReadBytes(reinterpret_cast<void*>(descriptor+0x54),&compressionInfo,sizeof(compressionInfo));
                        LOG_INFO("Maps","authenticating shader dependency '%s' as original '%s'",name,source.c_str());
                        SetLastError(saved);
                        // InitInflator D881C0 copies the name into its auth state.
                        // Kind 1 retains the native signature/hash verification.
                        return g_initInflator(state,1,reinterpret_cast<uintptr_t>(source.c_str()),compressionInfo?descriptor+0x54:0);
                    }
                }
            } catch(...) { }
        }
        if (safemem::ReadBytes(reinterpret_cast<void*>(reader),&state,sizeof(state)) && state &&
            safemem::ReadBytes(reinterpret_cast<void*>(reader+0xB0),&descriptor,sizeof(descriptor)) && descriptor &&
            safemem::ReadBytes(reinterpret_cast<void*>(descriptor+0x50),&signedFile,1) && !signedFile &&
            safemem::ReadBytes(reinterpret_cast<void*>(descriptor+0x88),header,sizeof(header)) &&
            memcmp(header,"IWffc100",8)==0 && header[8]==11 && header[9]==0 && header[10]==0 && header[11]==0 &&
            header[12]==0xF7 && header[13]==0x0F && header[14]==0 && header[15]==0 && header[16]==0 && header[17]==0)
        {
            const auto count = safemem::ReadString(reinterpret_cast<const char*>(descriptor),name,sizeof(name));
            try
            {
    ...
    HANDLE __fastcall OpenSelected(const char* filename, unsigned flags)
    {
        ...
        bool redirect = false;
        try { redirect = length && length < sizeof(request) - 1 && custommaps::ResolveDiskRead(request, replacement); }
        catch (...) { LOG_ERR("Maps", "selected package path resolution failed; native request retained"); }
        SetLastError(entryError);
        const HANDLE result = g_original.load(std::memory_order_acquire)(redirect ? replacement.c_str() : filename, flags);
...
namespace customloader
{
    hook::Status Install(uintptr_t base)
    {
        ...
        if (already) return hook::Status::Installed;
        uint8_t bytes[64]{};
        if (!safemem::ReadBytes(reinterpret_cast<void*>(base+replay::InitInflator.rva),bytes,replay::InitInflator.size) ||
            memcmp(bytes,replay::InitInflator.bytes,replay::InitInflator.size)!=0) return hook::Status::NotReady;
        ...
        status=hook::Install(reinterpret_cast<void*>(base+replay::ResetReader.rva),&ResetSelectedReader,
            replay::ResetReader.bytes,replay::ResetReader.size,g_reset);
        if (!already && status == hook::Status::Installed)
        {
            g_ready.store(true,std::memory_order_release);
            LOG_INFO("Maps", "selected package disk routing installed at Sys_OpenFileReliable RVA 0x13F4D50; native handle/read/close ownership retained");
        }
        return status;
    }
